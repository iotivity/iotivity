...
int gObservation = 0;
void * ChangeTemperatureRepresentation (void *param);
...
/// This class represents a single resource named 'temperatureResource'. This resource has
/// two simple properties unit 'range' and 'temperature'
...
public:
    /// Constructor
    TemperatureResource()
        :m_unit("C"), m_range("-273~500"), m_temperature(20), m_temperatureUri("/a/temperature"),
                m_resourceHandle(nullptr) {
        // Initialize representation
        m_temperatureRep.setUri(m_temperatureUri);
        
        m_temperatureRep.setValue("unit", m_unit);
        m_temperatureRep.setValue("range", m_range);
    ...
    void createResource()
    {
        ...
        //resource type name. In this case, it is temperature
    ...
    {
        // URI of the resource
        std::string resourceURI = "/a/temperature1";
        // resource type name. In this case, it is temperature
        std::string resourceTypeName = "oic.r.temperature";
        // resource interface.
        std::string resourceInterface = DEFAULT_INTERFACE;
    ...
    {
        try {
            if (rep.getValue("range", m_range))
            {
                cout << "\t\t\t\t" << "range: " << m_range << endl;
            }
            else
            {
                cout << "\t\t\t\t" << "range not found in the representation" << endl;
            }
    ...
    if(request)
    {
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            else if(requestType == "POST")
            {
                ...
                // Update the temperatureResource
                post(rep);
                pResponse->setErrorCode(200);
                pResponse->setResponseResult(OC_EH_OK);
                pResponse->setResourceRepresentation(get());
        ...
        if(requestFlag & RequestHandlerFlag::ObserverFlag)
        {
            ...
            // Observation happens on a different thread in ChangeTemperatureRepresentation function.
            // If we have not created the thread already, we will create one here.
            if(!startedThread)
            {
                pthread_create (&threadId, NULL, ChangeTemperatureRepresentation, (void *)this);
                startedThread = 1;
            }
            ehResult = OC_EH_OK;
        }
...
// ChangeTemperatureRepresentaion is an observation function,
// which notifies any changes to the resource to stack
// via notifyObservers
void * ChangeTemperatureRepresentation (void *param)
{
    TemperatureResource* temperaturePtr = (TemperatureResource*) param;
    ...
    while (1)
    {
        ...
        if (gObservation)
        {
            // If under observation if there are any changes to the temperature resource
            // we call notifyObservors
            //
            // For demostration we are changing the temperature value and notifying.
            temperaturePtr->m_temperature += 10;

            cout << "\nPower updated to : " << temperaturePtr->m_temperature << endl;
            cout << "Notifying observers with resource handle: " << temperaturePtr->getHandle() << endl;
            ...
            if(isListOfObservers)
            {
                ...
                resourceResponse->setResourceRepresentation(temperaturePtr->get(), DEFAULT_INTERFACE);

                result = OCPlatform::notifyListOfObservers(  temperaturePtr->getHandle(),
                                                             temperaturePtr->m_interestedObservers,
                                                             resourceResponse);
            }
            else
            {
                result = OCPlatform::notifyAllObservers(temperaturePtr->getHandle());
            }
...
void * handleSlowResponse (void *param, std::shared_ptr<OCResourceRequest> pRequest)
{
    ...
    pResponse->setResourceRepresentation(temperaturePtr->get());
    pResponse->setErrorCode(200);
    pResponse->setResponseResult(OC_EH_OK);
...
{
    std::cout << std::endl;
    std::cout << "Usage : temperatureserver <value>\n";
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // (in this case instance of class 'TemperatureResource').
        TemperatureResource myTemperature;

        // Invoke createResource function of class temperature.
        myTemperature.createResource();
        std::cout << "Created resource." << std::endl;

        myTemperature.addType(std::string("core.temperature"));
        myTemperature.addInterface(std::string(LINK_INTERFACE));
