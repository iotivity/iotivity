...
public:
    ...
    GW_RESULT refreshState();

    GW_RESULT setPower(bool power);

    /**
     * Sets the brightness for this bulb.
     * @param brightness Is in the range [0.0, 1.0].
     *        Values outside the interval are set to the nearest endpoint.
     * @return
     */
    GW_RESULT setBrightness(double brightness);

    GW_RESULT static getLights(const std::string user, std::vector<std::shared_ptr<LifxLight> > &lights);
...
private:
    ...
    GW_RESULT setState(std::string &setPowerRequest);
};
