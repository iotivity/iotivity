...
GW_RESULT parseCloudResponse(const std::string response)
{
    ...
    if (doc.HasMember("results"))
    {
        ...
        for (rapidjson::SizeType i = 0; i < results.Size(); ++i)
        {
            ...
            return status == "ok" ? GW_RESULT_OK : GW_RESULT_INTERNAL_ERROR;
        }
    }
    return GW_RESULT_OK;
}

GW_RESULT static parseLightsFromCloudResponse(const std::string response, const std::string user,
                                              std::vector<std::shared_ptr<LifxLight>> &parsedLights)
{
    ...
    for (rapidjson::Value::ConstValueIterator itr = lightsJsonResponse.Begin(); itr != lightsJsonResponse.End(); ++itr)
    {
...
GW_RESULT LifxLight::setState(std::string &stateRequest)
{
    if (this->user.empty())
    {
    ...
    CurlClient cc = CurlClient("PUT", uri + "/state")
...
GW_RESULT LifxLight::setPower(bool power)
{
    std::string setPowerRequest = std::string("power=") + (power? "on" : "off");
    return setState(setPowerRequest);
}

GW_RESULT LifxLight::setBrightness(double brightness)
{
...
GW_RESULT LifxLight::refreshState()
{
    if (this->user.empty())
    {
        throw std::runtime_error("Light not created in valid state by constructor. No \"user\" found");
    }
    CurlClient cc = CurlClient(CURL_GET, uri)
                .addRequestHeader(CURL_HEADER_ACCEPT_JSON)
                .setUserName(user);
    ...
    std::vector<std::shared_ptr<LifxLight>> parsedLights;
    GW_RESULT parseResult = parseLightsFromCloudResponse(response, this->user, parsedLights);

    if (parseResult != GW_RESULT_OK)
    {
        return parseResult;
    }
    ...
    {
        OIC_LOG_V(ERROR, TAG,"Found %lu lights instead of 1. This is irregular!", parsedLights.size());
        return GW_RESULT_JSON_ERROR;
    }
...
GW_RESULT LifxLight::getLights(const std::string user, std::vector<std::shared_ptr<LifxLight> > &lights)
{

    CurlClient cc = CurlClient(CURL_GET, LIFX_LIST_LIGHTS_URI)
                .addRequestHeader(CURL_HEADER_ACCEPT_JSON)
                .setUserName(user);
