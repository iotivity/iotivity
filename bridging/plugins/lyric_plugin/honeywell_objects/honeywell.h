...
#pragma pack(pop)
    ...
    /// Safely logs a string to GWLOG output. (GWLOG has a character limit; this ensures we never overflow
    /// the buffer. String is truncated if too long to safely display.)
    ///
    ...
    ///        File output is not truncated like GWLOG output is. Use this to capture JSON resonses from cloud.
    ...
    ///        device is used.)
    ///
    /// @return GW_RESULT_OK on success, error otherwise.
    virtual GW_RESULT setTemperature(LyricThermostatSharedPtr thermostat, const THERMOSTAT data,
...
protected:
    ...
    GW_RESULT manageMutexes(bool initialize);
