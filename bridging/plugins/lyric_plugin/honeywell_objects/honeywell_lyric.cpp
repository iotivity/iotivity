...
{
    // apparently there is no case-insensitive string compare in the standard library...
    ...
    return (strncmp(lowerStr1.c_str(), lowerStr2.c_str(), lowerStr1.length()));
}
...
int HoneywellLyric::getAccessToken(std::string &authorizationCode, ACCESS_TOKEN &accessToken)
{
    ...
    cc = CurlClient(CURL_POST, ACCESS_TOKEN_URL)
    ...
    if (fileExists(refreshTokenFile.c_str()))
    {
...
GW_RESULT HoneywellLyric::setTemperature(LyricThermostatSharedPtr thermostat, const THERMOSTAT data,
                                         const std::string uri)
{
    ...
    cc = CurlClient(CURL_POST, messageUrl)
                   .addRequestHeader(messageHeader)
...
int HoneywellLyric::getThermostats(std::vector<LyricThermostatSharedPtr> &thermostats)
{
    ...
    cc = CurlClient(CURL_GET, formattedUrl)
                   .addRequestHeader(messageHeader)
                   .setUserName(m_accessToken.accessToken);
                ...
                for (rapidjson::SizeType itr = 0; itr < devices.Size(); itr++)
                {
                    ...
                    if (devices[itr].HasMember(JSON_THERMOSTAT))
                    {
                        ...
                        if (thrmS.HasMember(JSON_CHANGEABLE_VALUES))
                        {
                            ...
                            if (0 == strcmpi(thermostatMode.c_str(), THERMOSTAT_MODE_COOL))
                            {
                                thermostatData.hvacMode = HVAC_COOL;
                            }
                            else if (0 == strcmpi(thermostatMode.c_str(), THERMOSTAT_MODE_HEAT))
                            {
                           ...
                           thermostatData.targetTempF =
                                          compute_target_temp(thermostatData.heatSetpointF, thermostatData.coolSetpointF);
