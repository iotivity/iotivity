...
GW_RESULT HueBridge::parseBridgeConfig(std::string json)
{
    if(json.empty())
    {
...
GW_RESULT HueBridge::getBridgeConfigFromCloud()
{
    rapidjson::Document doc;
    std::string discoveryUri;

    discoveryUri = m_curlQuery ;
    CurlClient cc = CurlClient(CURL_GET, discoveryUri)
    ...
    if (doc.IsObject())
    {
        std::string lightUri, configData;
        for (rapidjson::Value::ConstMemberIterator itr = doc.MemberBegin(); itr != doc.MemberEnd(); itr++)
        {
            ...
            if(lightUri == "config")
            {
                ...
                configData = sb.GetString();
                parseBridgeConfig(configData);
            }
        }
    }
    return GW_RESULT_OK;
...
GW_RESULT HueBridge::discoverHueLights()
{
    rapidjson::Document doc;
    std::string discoveryUri;
    ...
    if (doc.IsObject())
    {
        ...
        for (rapidjson::Value::ConstMemberIterator itr = doc.MemberBegin(); itr != doc.MemberEnd(); itr++)
        {
            ...
            std::shared_ptr<HueLight> light1 = std::make_shared<HueLight>(discoveryUri + lightUri, m_bridgeData.ip, m_bridgeData.id, lightUri, lightData);
