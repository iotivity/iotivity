 ...
 */
pthread_mutex_t g_discovered_bridges_lock;
std::vector<HueDiscoveredCtx> g_discovered_bridges;
 ...
 */
GW_RESULT hueInit(plugin_ctx_t *ctx, addAuthBridgeCallback addBridgeCb,
 ...
 */
GW_RESULT DiscoverRemoteBridges()
{
    GW_RESULT result = GW_RESULT_INTERNAL_ERROR;

    CurlClient cc = CurlClient(CURL_GET, BRIDGE_NUPNP_URI)
 ...
 *    GW_RESULT_INTERNAL_ERROR    - mechanics of sending out discovery failed
 */
GW_RESULT DiscoverLocalBridges()
{
    int sock;
    int err;
    ssize_t len;
...
/* Discovere both local and Remote Hue Bridges */

GW_RESULT DiscoverHueBridges()
{
    GW_RESULT result = GW_RESULT_INTERNAL_ERROR;

    GW_RESULT resultRemote = DiscoverRemoteBridges();
    GW_RESULT resultLocal = DiscoverLocalBridges();
...
GW_RESULT hueAuthDestroy()
{
    ...
    if (g_hueAuthCtx.structInitialized == true)
    {
        ...
        for (std::vector<HueDiscoveredCtx>::iterator it = g_discovered_bridges.begin() ; it != g_discovered_bridges.end(); ++it)
        {
        ...
        pthread_mutex_destroy(&g_discovered_bridges_lock);
        g_discovered_bridges.clear();
    }
...
void initializeHueAuthCtx()
{
    memset(&g_discovered_bridges_lock, 0, sizeof(pthread_mutex_t));
    if (pthread_mutex_init(&g_discovered_bridges_lock, NULL) != 0)
    {
...
/* Add bridge to the discovered bridge vector
 *
 * returns:
...
{
    g_discovered_bridges.push_back(discoveredCtx);
    return GW_RESULT_OK;
}
