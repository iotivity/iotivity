...
public:
    ...
    typedef struct _THERMOSTAT
    {
        ...
        _THERMOSTAT()
        {
            isOnline = canCool = canHeat = usingEmergencyHeat = hasFan = fanTimerActive = fanTimerTimeout = hasLeaf = false;
            targetTempC = targetTempHighC = targetTempLowC = awayTempHighF = awayTempHighC = ambientTempF = ambientTempC = 0.0;
    ...
    int setTemperature(uint16_t targetTemp);
