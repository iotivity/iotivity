...
Also, don't be evil. Give valid inputs!

-n          Start plugins in NON secure mode.Default is secure.

-a          Start this client in auto add mode.
            This will cause the client send an ADD message with the same message as in the scan response.
...
enum pluginCommands { SCAN, ADD, REMOVE, RECONNECT, UNLOAD, MAX_COMMANDS };
...
void fileWriteAndDbUpdate(const char * uri, const char * pluginName, const char * metadata)
{
    ...
    fwrite(metadata, 1, MAX_METADATA_LEN, fp);
    fputs("\n", fp);
    fclose(fp);
    ...
    rc = sqlite3_bind_text(stmt, BIND_INDEX_FIRST, uri, MAX_URI_LEN, SQLITE_STATIC);
    PDM_VERIFY_SQLITE_OK(TAG, rc, ERROR);
...
void deleteReconnectFile(char * uri)
{
    ...
    res = sqlite3_bind_text(stmt, BIND_INDEX_FIRST, uri, MAX_URI_LEN, SQLITE_STATIC);
    PDM_VERIFY_SQLITE_OK(TAG, res, ERROR);
...
{
    char * uri = (char *) calloc(1, MAX_URI_LEN);
...
GW_CB_RESULT onCallback(uint32_t msgType, MPMMessage message, size_t size, const char * plugin_name)
{
    (void) msgType;
    ...
    else if (msgType == MPM_ADD)
    {
        ...
        memset(&addResponse, 0, sizeof(add_response));
        memcpy(&addResponse, message_char, sizeof(add_response));
...
void loadPlugin(const std::string pluginPath)
{
    ...
    GW_RESULT result = MPMLoad(&pluginHandle, pluginPath.c_str(), onCallback, NULL);
...
void printCommands()
{
    ...
    , RECONNECT , ":" , "RECONNECT  ", UNLOAD , ":" , "UNLOAD" , "\n");
}
...
void addCommand(MPMPluginHandle handle)
{
    ...
    char* addMessage = (char*) malloc(size);
    memcpy(addMessage, message.c_str(), size);

    log("Sending ", addMessage, "\n");
    MPMAddDevice(handle, addMessage, size);
...
{
    std::string message;
    log("What message to send to REMOVE (Usually URI of a \"scanned\" resource) : ");
    std::cin >> message;

    size_t size = message.size() + 1;
    char* removeMessage = (char*) malloc(size);
    memcpy(removeMessage, message.c_str(), size);

    log("Sending ", removeMessage, "\n");
    MPMRemoveDevice(handle, removeMessage, size);
...
void reconnect()
{
             ...
             ++itr)
    {
        ...
        while (SQLITE_ROW == sqlite3_step(stmt))
        {
            ...
            fclose(fp);
            MPMReconnectDevice(itr->second, metadata, MAX_METADATA_LEN);
        }
