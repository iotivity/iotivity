...
public class EasySetupServiceTest extends AndroidTestCase {
    ...
    public void testStartSetupWithBleOnboarding() {

        final String UUID = "ade3d529-c784-4f63-a987-eb69f70ee816";
        ...
        mService = EasySetupService.getInstance(getContext(), new EasySetupStatus() {
            ...
            public void onFinished(EnrolleeDevice enrolledevice) {
                ...
                if (enrolledevice.isSetupSuccessful()) {
                    ...
                    if (enrolledevice.mOnBoardingConfig.getConnType() == WiFiOnBoardingConfig.ConnType.BLE) {
                        BleConnection conn = (BleConnection) enrolledevice.getConnection();
                        ...
                        String uuid = conn.getmServiceUUID();
                        if (uuid == null || uuid.isEmpty()) {
                            assertTrue(false);
                            return;
                        }

                        //  Conform that device are same
                        if (!uuid.equals(UUID)) {
                            assertTrue(false);
                            return;
                        }
        ...
        /* Create enrolling device */
        mDevice = mFactory.newEnrolleeDevice(bleOnBoardingConfig, mWiFiProvConfig);
        ...
        /* Check if the the correct device is created as per the given configuration*/
        assertTrue((mDevice instanceof EnrolleeDeviceBLEOnBoarding));
        ...
        try {
            ...
            BleConnection conn = (BleConnection) mDevice.getConnection();
            if (conn == null) {
                assertTrue(false);
                return;
            }
            ...
            Log.i("EasySetupTest", "MAC " + mac);
            Log.i("EasySetupTest", "UUID " + uuid);
