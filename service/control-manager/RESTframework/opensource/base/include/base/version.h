...
// BaseVersion represents a dotted version number, like "1.2.3.4", supporting
// parsing and comparison.
class BASE_EXPORT BaseVersion {
 public:
  // The only thing you can legally do to a default constructed
  // BaseVersion object is assign to it.
  BaseVersion();
  ...
  // the outcome.
  explicit BaseVersion(const std::string& version_str);
  ...
  // Caller is responsible for freeing the BaseVersion object once done.
  // DO NOT USE FOR NEWER CODE.
  static BaseVersion* GetVersionFromString(const std::string& version_str);

  // Creates a copy of this version object. Caller takes ownership.
  // DO NOT USE FOR NEWER CODE.
  BaseVersion* Clone() const;
  
  bool Equals(const BaseVersion& other) const;

  // Returns -1, 0, 1 for <, ==, >.
  int CompareTo(const BaseVersion& other) const;
