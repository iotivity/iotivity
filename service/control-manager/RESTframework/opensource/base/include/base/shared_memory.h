...
struct SharedMemoryCreateOptions {
  SharedMemoryCreateOptions() : name(NULL), size(0), open_existing(false),
                                executable(false), shm_path(NULL) {}
  ...
  // Added by SRI-B : To create shared memory in a specified path else use default path
...
class BASE_EXPORT SharedMemory {
 public:
  ...
  bool Delete(const std::string& name, const std::string* shm_path = NULL);

  // Opens a shared memory segment based on a name.
  // If read_only is true, opens for read-only access.
  // Returns true on success, false on failure.
  bool Open(const std::string& name, bool read_only, const std::string* shm_path = NULL);
 ...
 private:
  ...
  bool FilePathForMemoryName(const std::string& mem_name, FilePath* path, const std::string* shm_path);
