...
#if defined(SLEP_TARGET_PLATFORM_GUGGENHEIM) || defined(SLEP_TARGET_PLATFORM_TIZEN)
  // work-around of gettimeofday() bug in Guggenheim TV
  // which always returns year 1970.
  // FromUTCExploded() or FromLocalExploded() should be used to
  // get Time input from calendar time.
  static bool SetNow(Time t);
  // below we don't have  second struct timezone *tz parameter
  // since base module code does not use it.
  static int  gettimeofday_Guggenheim(struct timeval *tv);
private:
  // below will contain the value = ( value which gettimeofday must have returned
  // when SetNow() is called  ) - (monotonic_time value from
  // clock_gettime(CLOCK_MONOTONIC, &monotonic_time) call )
  static int64 guggenheim_tv_app_start_time_;
public:
#endif
