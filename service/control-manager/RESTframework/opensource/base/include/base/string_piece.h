...
template <typename STRING_TYPE> class StringPieceDetail {
 ...
 public:
  ...
  StringPieceDetail(const typename STRING_TYPE::const_iterator& _begin,
                    const typename STRING_TYPE::const_iterator& _end)
      : ptr_((_end > _begin) ? &(*_begin) : NULL),
        length_((_end > _begin) ? (size_type)(_end - _begin) : 0) {}
  ...
  void set(const value_type* _data, size_type _len) {
    ptr_ = _data;
    length_ = _len;
  }
...
template <> class BasicStringPiece<std::string> :
 ...
 public:
  ...
  BasicStringPiece(const std::string::const_iterator& _begin,
                   const std::string::const_iterator& _end)
      : internal::StringPieceDetail<std::string>(_begin, _end) {}

  // Prevent the following overload of set() from hiding the definitions in the
  // base class.
  using internal::StringPieceDetail<std::string>::set;

  void set(const void* _data, size_type _len) {
    ptr_ = reinterpret_cast<const value_type*>(_data);
    length_ = _len;
  }
