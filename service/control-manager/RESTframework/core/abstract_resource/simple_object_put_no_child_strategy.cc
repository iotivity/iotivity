...
namespace webservice
{
    ...
    /**
      ...
      */
    HTTPResponseType SimpleObjectPutNoChildStrategy::FindAndProcessChildObject(
    ...
    {
        DLOG(ERROR) << "No Child Strategy for Simple Object";
        ...
        if (web_util::HTTP_METHOD_PUT == method)
        {
            if (base::Value::TYPE_LIST != request_body->GetType())
            {
                return FindAndProcessChildObjectForLinkedChild(request_line, request_header, request_body,
    ...
    /**
      * Find child object & Process child object for linked child
      ...
      * @return HTTPResponseType
      */
    HTTPResponseType SimpleObjectPutNoChildStrategy::FindAndProcessChildObjectForLinkedChild(
        const HTTPRequestLine &request_line, const HTTPRequestHeader &request_header,
        base::Value *request_body,
        HTTPResponseHeader &response_header, base::Value *response_body,
        std::map<std::string, std::string> &id_deps, const std::string &current_uri,
        BaseSimpleObject *p_simple_obj)
    {
        DLOG(ERROR) << "No Child Strategy for Linked Child";
        if (NULL == p_simple_obj)
        {
            LOG(ERROR) << "Input instance is NULL!";
            return RT_500_ALLOC_FAIL;
        ...
        for (it = child_map.begin(); it != child_map.end(); it++)
        {
            ...
            if (p_child_obj->ParseChildRequestBody(http_method, request_body, &request_child, ""))
            {
                if (enSimpleLinkedChild == p_child_obj->GetSimpleObjectType())
                {
                ...
                {
                    response_error_code = RT_400_INCORRECT_FIELD;
                }
            }
    ...
    /**
      ...
      * @return HTTPResponseType
      */
    HTTPResponseType SimpleObjectPutNoChildStrategy::FindAndProcessChildObjectForIdentifiedObject(
        const HTTPRequestLine &request_line, const HTTPRequestHeader &request_header,
        base::Value *request_body,
        HTTPResponseHeader &response_header, base::Value *response_body,
        std::map<std::string, std::string> &id_deps, const std::string &current_uri,
        BaseSimpleObject *p_simple_obj)
    {
        DLOG(ERROR) << "No Child Strategy for Identified Object";
        if (NULL == p_simple_obj)
        {
            LOG(ERROR) << "Input instance is NULL!";
            return RT_500_ALLOC_FAIL;
        ...
        for (it = child_map.begin(); it != child_map.end(); it++)
        {
            ...
            if (enSimpleIdentifiedObject == p_child_obj->GetSimpleObjectType())
            {
                ...
                if (0 == p_simple_obj->GetObjectIDList(child_object_name, &id_list,
                                                       &http)) return RT_404_NOT_FOUND_OBJECT_ID_LIST;

                std::vector<std::string>::iterator id_list_iter;
                for (id_list_iter = id_list.begin(); id_list_iter != id_list.end(); id_list_iter++)
                {
                    base::Value *request_child = NULL;
                    if (p_simple_obj->ParseChildRequestBody(http_method, request_body, &request_child, *id_list_iter))
                    {
