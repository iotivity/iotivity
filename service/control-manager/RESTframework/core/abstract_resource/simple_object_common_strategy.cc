...
namespace webservice
{
    ...
    /**
      ...
      */
    HTTPResponseType SimpleObjectCommonStrategy::FindAndProcessChildObject(
        ...
        for (it = child_map.begin(); it != child_map.end(); it++)
        {
            ...
            if (enSimpleIdentifiedObject != p_child_obj->GetSimpleObjectType())
            {
                ...
                if (p_child_obj->ParseChildRequestBody(http_method, request_body, &request_child, ""))
                {
                    ...
                    DLOG(INFO) << "found_resource_uri : " << found_uri_add_child << ", request_child addr : " <<
                               request_child;
            ...
            else
            {
                ...
                if (0 != p_simple_obj->GetObjectIDList(child_object_name, &id_list, &http_message))
                {
            ...
            // If response is error, immediately return
