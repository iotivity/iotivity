            ...
            namespace Serialization
            {
                namespace Json
                {
                    ...
                    JsonSerializer::JsonSerializer()
                    {
                        ...
                        CM_LOG_INFO_A(CM_JSON, "%s", "starting the value list to store the json object");
                    }
...
//}
                    ...
                    JsonSerializer::getOutput(string &output, int &length)
                    {
                        if ((m_values.size() == 1) && (m_currentNodes.size() == 0))
                        {
                            ...
                            CM_LOG_DEBUG_A(CM_JSON, "Json Serialization Output: %s", output.c_str());
                            CM_LOG_DEBUG_A(CM_JSON, "Json Serialization size: %d", length);
...
//      }
                            ...
                            if (m_values.size() > 0)
                            {
                                if (((value->empty() == false) || (currentLevel <= 1) ))
                                {
                                    ...
                                    {
                                        if (((*(m_values.back()))[nodeName].isNull() == false)
                                            &&  ((*(m_values.back()))[nodeName].isArray()))
                                        {
...
//                          }
                    ...
                    {
                        return CM_CONTENT_TYPE_JSON_UTF8;
                    }
                    ...
                    JsonSerializer::endListProperty(string nodeName)
                    {
                        ...
                        if (((*(m_values.back()))[nodeName].isNull() == false)
                            && ((*(m_values.back()))[nodeName].isArray()) && ((*(m_values.back()))[nodeName].empty()))
                        {
