...
Messages::~Messages()
{
    for ( std::list< ::Message * >::const_iterator itr = mpMessage.begin(); itr != mpMessage.end();
          itr++ )
    {
        delete (*itr);
    }
...
bool
Messages::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...
bool
Messages::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Messages") == false) ) { deserializer.setErrorMessage("Failed To deserialize : Messages"); return false; }
            }
        }
    }
...
std::string
Messages::getElementName()
{
    return EN_MESSAGES;
}

bool
Messages::clone(::Messages *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpMessage.clear();
        for ( std::list< ::Message * >::const_iterator itr = pSrcObject->mpMessage.begin();
