...
bool
UsageLimits::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...
bool
UsageLimits::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("UsageLimits") == false) ) { deserializer.setErrorMessage("Failed To deserialize : UsageLimits"); return false; }
            }
        }
    }
...
std::string
UsageLimits::getElementName()
{
    return EN_USAGELIMITS;
}

bool
UsageLimits::clone(::UsageLimits *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpUsageLimit.clear();
        for ( std::list< ::UsageLimit * >::const_iterator itr = pSrcObject->mpUsageLimit.begin();
              itr != pSrcObject->mpUsageLimit.end(); itr++ )
        {
            ::UsageLimit *pTemp = new ::UsageLimit();
            if (false == pTemp->clone((*itr)))
            {
                delete pTemp;//10112014_PREVENT_287877
                return false;
            } ;
