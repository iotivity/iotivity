...
bool
AccessPoints::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        if ( mpAccessPoint.size() > 0)
        {
            ...
            if (false == bRet) { if (serializer.isInFilteringMode("AccessPoints") == false) {serializer.setErrorMessage("Failed To Serialize parameter (Invalid content) : AccessPoints"); return false; }}
        }
...
bool
AccessPoints::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("AccessPoints") == false) ) { deserializer.setErrorMessage("Failed To deserialize : AccessPoints"); return false; }
            }
        }
    }
...
std::string
AccessPoints::getElementName()
{
    return EN_ACCESSPOINTS;
}

bool
AccessPoints::clone(::AccessPoints *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpAccessPoint.clear();
        for ( std::list< ::AccessPoint * >::const_iterator itr = pSrcObject->mpAccessPoint.begin();
