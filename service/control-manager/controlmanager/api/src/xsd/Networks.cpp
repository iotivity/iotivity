...
Networks::~Networks()
{
    for ( std::list< ::Network * >::const_iterator itr = mpNetwork.begin(); itr != mpNetwork.end();
          itr++ )
    {
        delete (*itr);
    }
...
bool
Networks::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...
bool
Networks::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Networks") == false) ) { deserializer.setErrorMessage("Failed To deserialize : Networks"); return false; }
            }
        }
    }
...
std::string
Networks::getElementName()
{
    return EN_NETWORKS;
}

bool
Networks::clone(::Networks *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpNetwork.clear();
        for ( std::list< ::Network * >::const_iterator itr = pSrcObject->mpNetwork.begin();
