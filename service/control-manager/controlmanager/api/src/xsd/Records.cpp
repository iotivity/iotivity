...
Records::~Records()
{
    for ( std::list< ::Record * >::const_iterator itr = mpRecord.begin(); itr != mpRecord.end();
          itr++ )
    {
        delete (*itr);
    }
...
bool
Records::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        if ( mpRecord.size() > 0)
        {
            ...
            if (false == bRet) { if (serializer.isInFilteringMode("Records") == false) {serializer.setErrorMessage("Failed To Serialize parameter (Invalid content) : Records"); return false; }}
        }
...
bool
Records::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Records") == false) ) { deserializer.setErrorMessage("Failed To deserialize : Records"); return false; }
            }
        }
    }
...
std::string
Records::getElementName()
{
    return EN_RECORDS;
}

bool
Records::clone(::Records *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpRecord.clear();
        for ( std::list< ::Record * >::const_iterator itr = pSrcObject->mpRecord.begin();
              itr != pSrcObject->mpRecord.end(); itr++ )
        {
