...
Doors::~Doors()
{
    for ( std::list< ::Door * >::const_iterator itr = mpDoor.begin(); itr != mpDoor.end(); itr++ )
    {
        delete (*itr);
    }
    mpDoor.clear();
...
bool
Doors::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...
bool
Doors::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Doors") == false) ) { deserializer.setErrorMessage("Failed To deserialize : Doors"); return false; }
            }
        }
    }
...
std::string
Doors::getElementName()
{
    return EN_DOORS;
}

bool
Doors::clone(::Doors *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
...

    {
        ...
        mpDoor.clear();
        for ( std::list< ::Door * >::const_iterator itr = pSrcObject->mpDoor.begin();
              itr != pSrcObject->mpDoor.end(); itr++ )
        {
            ::Door *pTemp = new ::Door();
