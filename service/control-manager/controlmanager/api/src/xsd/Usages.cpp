...
Usages::~Usages()
{
    ...
    if ( NULL != mpPeriodType) { delete mpPeriodType; mpPeriodType = NULL; }
    ...
    for ( std::list< ::Usage * >::const_iterator itr = mpUsage.begin(); itr != mpUsage.end(); itr++ )
    {
        delete (*itr);
    }
    mpUsage.clear();
...
bool
Usages::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
    ...
    }
    {
        ...
        if ( NULL != mpPeriodType )
        {
            if (mpPeriodType->value.length() > 0)
            {
                ...
                {
                    bRet = serializer.setPropertyValue("periodType", mpPeriodType->value);
                }
                else { bRet = false ;}
            }
            else { bRet = false;}
        }
        if (false == bRet) { if (serializer.isInFilteringMode("periodType") == false) {serializer.setErrorMessage("Failed To Serialize parameter (Invalid content) : periodType"); return false; }}
    }
    {
        bool bRet = true;
    ...
    }
    {
        ...
        if ( mpUsage.size() > 0)
        {
            ...
            if (false == bRet) { if (serializer.isInFilteringMode("Usage") == false) {serializer.setErrorMessage("Failed To Serialize parameter (Invalid content) : Usage"); return false; }}
...
bool
Usages::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
    ...
    if (true == deserializer.hasProperty("periodType"))
    {
        ...
        if (mpPeriodType == NULL ) { deserializer.setErrorMessage("Failed to allocate memrory for parameter :  periodType"); return false;}
        if (false == deserializer.getPropertyValue("periodType",
                &(mpPeriodType->value))) {deserializer.setErrorMessage("Failed to deserialize parameter :  periodType"); return false ;}
        if (false == mpPeriodType->validateContent()) { deserializer.setErrorMessage("Parameter Contains Invalid Value : periodType"); return false ;}
    }
    else { if (deserializer.isInFilteringMode("periodType") == false) { deserializer.setErrorMessage("Mandatory Parameter not found : periodType"); return false; }}
    ...
    if (true == deserializer.hasProperty("deviceID"))
    {
        ...
        if (false == deserializer.getPropertyValue("deviceID",
                &(mpDeviceID->value))) {deserializer.setErrorMessage("Failed to deserialize parameter :  deviceID"); return false ;}
    ...
    if (true == deserializer.hasProperty("groupID"))
    {
        ...
        if (false == deserializer.getPropertyValue("groupID",
                &(mpGroupID->value))) {deserializer.setErrorMessage("Failed to deserialize parameter :  groupID"); return false ;}
    ...
    }
    {
        ...
        else
        {
            if (true == deserializer.hasProperty("Usage"))
            {
                ::Usage *m = new ::Usage();
                if (NULL != m)
                    if (true == m->deSerialize(deserializer))
                        mpUsage.push_back(m);
            }
            else
            {
                if ((deserializer.isInFilteringMode("Usage") == false) )
                {
                    deserializer.setErrorMessage("Failed To deserialize : Usage");
                }
                return false;
            }
        }
...
std::string
Usages::getElementName()
{
    return EN_USAGES;
}

bool
Usages::clone(::Usages *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false ;
    ...
    }
    {
        ...
        {
            mpPeriodType = new ::PeriodType();
            if (mpPeriodType == NULL ) {return false;}
    ...
    }
    {
        ...
        mpUsage.clear();
        for ( std::list< ::Usage * >::const_iterator itr = pSrcObject->mpUsage.begin();
              itr != pSrcObject->mpUsage.end(); itr++ )
        {
