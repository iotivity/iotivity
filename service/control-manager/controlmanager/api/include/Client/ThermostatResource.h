...
#ifndef THERMOSTATRESOURCE_H_
#define THERMOSTATRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IThermostatResourceResponseListener.h"
...
/**
 * @class   ThermostatResource
 * @brief   ThermostatResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class ThermostatResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of ThermostatResource class.
         *
         ...
         */
        ThermostatResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of ThermostatResource class.
         */
        virtual ~ThermostatResource();
         ...
         *
         * @param[in]   ThermostatResourceResponseListener  Object that implements IThermostatResourceResponseListener interface
         */
        void addResponseListener (IThermostatResourceResponseListener &respListener) ;

        IThermostatResourceResponseListener *getResponseListener ();

        bool getThermostat( int &requestId );


        bool putThermostat( int &requestId , ::Thermostat &reqData);


        void buildPath(std::string deviceId);

    private:
        class ThermostatResourceListenerInternal : public
            OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                ThermostatResource *resource;

                ThermostatResourceListenerInternal(ThermostatResource *res)
                {
                    resource = res;
                }

                ~ThermostatResourceListenerInternal()
                {
                }
                ...
                {
                    IThermostatResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_THERMOSTAT))
                            {
                                ::Thermostat *respData = (::Thermostat *)response ;
                                p_respListener->ongetThermostat(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetThermostat(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetThermostat(clientSessionId, status,  NULL) ;
                        }
                    }
                }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputThermostat( clientSessionId, status ) ;
                    }
                }
                void
        ...
        /**
         * Variable that contains IThermostatResourceResponseListener object.
         */
        IThermostatResourceResponseListener *p_respListener ;

        OC::Cm::Client::Resource::IResourceResponseListener *p_internalResponseListener ;

};
/** @} */
#endif /* THERMOSTATRESOURCE_H_ */
