...
#ifndef TEMPERATURESRESOURCE_H_
#define TEMPERATURESRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "ITemperaturesResourceResponseListener.h"
...
/**
 * @class   TemperaturesResource
 * @brief   TemperaturesResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class TemperaturesResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of TemperaturesResource class.
         *
         ...
         */
        TemperaturesResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of TemperaturesResource class.
         */
        virtual ~TemperaturesResource();
         ...
         *
         * @param[in]   TemperaturesResourceResponseListener    Object that implements ITemperaturesResourceResponseListener interface
         */

        void addResponseListener (ITemperaturesResourceResponseListener &respListener) ;

        ITemperaturesResourceResponseListener *getResponseListener ();

        bool getTemperatures( int &requestId );


        bool putTemperatures( int &requestId , ::Temperatures &reqData);


        void buildPath(std::string deviceId);

    private:
        class TemperaturesResourceListenerInternal : public
            OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                TemperaturesResource *resource;

                TemperaturesResourceListenerInternal(TemperaturesResource *res)
                {
                    resource = res;
                }

                ~TemperaturesResourceListenerInternal()
                {
                }
                ...
                {
                    ITemperaturesResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_TEMPERATURES))
                            {
                                ::Temperatures *respData = (::Temperatures *)response ;
                                p_respListener->ongetTemperatures(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetTemperatures(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetTemperatures(clientSessionId, status,  NULL) ;
                        }
                    }
                }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputTemperatures( clientSessionId, status ) ;
                    }
                }
                void
        ...
        /**
         * Variable that contains ITemperaturesResourceResponseListener object.
         */
        ITemperaturesResourceResponseListener *p_respListener ;

        OC::Cm::Client::Resource::IResourceResponseListener *p_internalResponseListener ;

};
/** @} */
#endif /* TEMPERATURESRESOURCE_H_ */
