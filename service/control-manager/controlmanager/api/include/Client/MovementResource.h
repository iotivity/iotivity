...
#ifndef MOVEMENTRESOURCE_H_
#define MOVEMENTRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IMovementResourceResponseListener.h"
 ...
 * @brief   MovementResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class MovementResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of MovementResource class.
         *
         ...
         */
        MovementResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of MovementResource class.
         */
        virtual ~MovementResource();
         ...
         *
         * @param[in]   MovementResourceResponseListener    Object that implements IMovementResourceResponseListener interface
         */

        void addResponseListener (IMovementResourceResponseListener &respListener) ;

        IMovementResourceResponseListener *getResponseListener ();

        bool getMovement( int &requestId );


        bool putMovement( int &requestId , ::Movement &reqData);


        void buildPath(std::string deviceId);

    private:
        class MovementResourceListenerInternal : public OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                MovementResource *resource;

                MovementResourceListenerInternal(MovementResource *res)
                {
                    resource = res;
                }

                ~MovementResourceListenerInternal()
                {
                }
                ...
                {
                    IMovementResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_MOVEMENT))
                            {
                                ::Movement *respData = (::Movement *)response ;
                                p_respListener->ongetMovement(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetMovement(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetMovement(clientSessionId, status,  NULL) ;
                        }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputMovement( clientSessionId, status ) ;
                    }
                }
                void
        ...
        /**
         * Variable that contains IMovementResourceResponseListener object.
         */
        IMovementResourceResponseListener *p_respListener ;

        OC::Cm::Client::Resource::IResourceResponseListener *p_internalResponseListener ;

};
/** @} */
#endif /* MOVEMENTRESOURCE_H_ */
