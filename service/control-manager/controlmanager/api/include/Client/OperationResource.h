...
#include "IOperationResourceResponseListener.h"
 ...
 * @brief   OperationResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class OperationResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of OperationResource class.
         *
         ...
         */
        OperationResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of OperationResource class.
         */
         ...
         *
         * @param[in]   OperationResourceResponseListener   Object that implements IOperationResourceResponseListener interface
         */

        void addResponseListener (IOperationResourceResponseListener &respListener) ;

        IOperationResourceResponseListener *getResponseListener ();

        bool getOperation( int &requestId );


        bool putOperation( int &requestId , ::Operation &reqData);


        void buildPath(std::string deviceId);

    private:
        class OperationResourceListenerInternal : public OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                OperationResource *resource;

                OperationResourceListenerInternal(OperationResource *res)
                {
                    resource = res;
                }

                ~OperationResourceListenerInternal()
                {
                }
                ...
                {
                    IOperationResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_OPERATION))
                            {
                                ::Operation *respData = (::Operation *)response ;
                                p_respListener->ongetOperation(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetOperation(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetOperation(clientSessionId, status,  NULL) ;
                        }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputOperation( clientSessionId, status ) ;
                    }
                }
                void
        ...
        /**
         * Variable that contains IOperationResourceResponseListener object.
         */
        IOperationResourceResponseListener *p_respListener ;
