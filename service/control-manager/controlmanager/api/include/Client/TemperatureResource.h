...
#ifndef TEMPERATURERESOURCE_H_
#define TEMPERATURERESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "ITemperatureResourceResponseListener.h"
 ...
 * @brief   TemperatureResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class TemperatureResource : public OC::Cm::Client::Resource::Resource
{
    ...
    public:
         ...
         */
        TemperatureResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of TemperatureResource class.
         */
         ...
         *
         * @param[in]   TemperatureResourceResponseListener Object that implements ITemperatureResourceResponseListener interface
         */

        void addResponseListener (ITemperatureResourceResponseListener &respListener) ;

        ITemperatureResourceResponseListener *getResponseListener ();

        bool getTemperature( int &requestId );


        bool putTemperature( int &requestId , ::Temperature &reqData);


        void buildPath(std::string deviceId, std::string temperatureId);

    private:
        class TemperatureResourceListenerInternal : public
            OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:
                ...
                TemperatureResourceListenerInternal(TemperatureResource *res)
                {
                    resource = res;
                }

                ~TemperatureResourceListenerInternal()
                {
                }
                ...
                {
                    ITemperatureResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_TEMPERATURE))
                            {
                                ::Temperature *respData = (::Temperature *)response ;
                                p_respListener->ongetTemperature(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                        ...
                        {
                            p_respListener->ongetTemperature(clientSessionId, status,  NULL) ;
                        }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputTemperature( clientSessionId, status ) ;
                    }
                }
                void
