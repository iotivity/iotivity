...
#ifndef ACCESSPOINTRESOURCE_H_
#define ACCESSPOINTRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IAccessPointResourceResponseListener.h"
 ...
 * @brief   AccessPointResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class AccessPointResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of AccessPointResource class.
         *
         ...
         */
        AccessPointResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of AccessPointResource class.
         */
        virtual ~AccessPointResource();
         ...
         *
         * @param[in]   AccessPointResourceResponseListener Object that implements IAccessPointResourceResponseListener interface
         */

        void addResponseListener (IAccessPointResourceResponseListener &respListener) ;

        IAccessPointResourceResponseListener *getResponseListener ();


        bool getAccessPoint( int &requestId );


        void buildPath(std::string deviceId, std::string networkId, std::string accesspointId);


    private:

        class AccessPointResourceListenerInternal : public
            OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                AccessPointResource *resource;

                AccessPointResourceListenerInternal(AccessPointResource *res)
                {
                    resource = res;
                }

                ~AccessPointResourceListenerInternal()
                {
                }
                ...
                {
                    IAccessPointResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_ACCESSPOINT))
                            {
                                ::AccessPoint *respData = (::AccessPoint *)response ;
                                p_respListener->ongetAccessPoint(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetAccessPoint(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetAccessPoint(clientSessionId, status,  NULL) ;
                        }
        ...
        /**
         * Variable that contains IAccessPointResourceResponseListener object.
         */
        IAccessPointResourceResponseListener *p_respListener ;
