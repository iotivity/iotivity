...
#ifndef USAGESRESOURCE_H_
#define USAGESRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IUsagesResourceResponseListener.h"
...
/**
 * @class   UsagesResource
 * @brief   UsagesResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class UsagesResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of UsagesResource class.
         *
         ...
         */
        UsagesResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of UsagesResource class.
         */
        virtual ~UsagesResource();
         ...
         *
         * @param[in]   UsagesResourceResponseListener  Object that implements IUsagesResourceResponseListener interface
         */

        void addResponseListener (IUsagesResourceResponseListener &respListener) ;

        IUsagesResourceResponseListener *getResponseListener ();

        bool getUsages( int &requestId );


        void buildPath( );

    private:
        class UsagesResourceListenerInternal : public OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                UsagesResource *resource;

                UsagesResourceListenerInternal(UsagesResource *res)
                {
                    resource = res;
                }

                ~UsagesResourceListenerInternal()
                {
                }
                ...
                {
                    IUsagesResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_USAGES))
                            {
                                ::Usages *respData = (::Usages *)response ;
                                p_respListener->ongetUsages(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetUsages(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetUsages(clientSessionId, status,  NULL) ;
                        }
        ...
        /**
         * Variable that contains IUsagesResourceResponseListener object.
         */
        IUsagesResourceResponseListener *p_respListener ;

        OC::Cm::Client::Resource::IResourceResponseListener *p_internalResponseListener ;

};
/** @} */
#endif /* USAGESRESOURCE_H_ */
