...
#ifndef WIFIRESOURCE_H_
#define WIFIRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IWiFiResourceResponseListener.h"
...
/**
 * @class   WiFiResource
 * @brief   WiFiResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class WiFiResource : public OC::Cm::Client::Resource::Resource
{
    ...
    public:
         ...
         */
        WiFiResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of WiFiResource class.
         */
         ...
         *
         * @param[in]   WiFiResourceResponseListener    Object that implements IWiFiResourceResponseListener interface
         */

        void addResponseListener (IWiFiResourceResponseListener &respListener) ;

        IWiFiResourceResponseListener *getResponseListener ();

        bool getWiFi( int &requestId );


        bool putWiFi( int &requestId , ::WiFi &reqData);
    ...
    private:
        class WiFiResourceListenerInternal : public OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                WiFiResource *resource;

                WiFiResourceListenerInternal(WiFiResource *res)
                {
                    resource = res;
                }

                ~WiFiResourceListenerInternal()
                {
                }
                ...
                {
                    IWiFiResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_WIFI))
                            {
                                ::WiFi *respData = (::WiFi *)response ;
                                p_respListener->ongetWiFi(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                        ...
                        {
                            p_respListener->ongetWiFi(clientSessionId, status,  NULL) ;
                        }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputWiFi( clientSessionId, status ) ;
                    }
                }
                void
