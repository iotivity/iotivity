...
#ifndef DETECTIONRESOURCE_H_
#define DETECTIONRESOURCE_H_

#include "CMClientResource.h"
#include "xsd/Elements.h"

#include "IDetectionResourceResponseListener.h"
 ...
 * @brief   DetectionResource class implements OC::Cm::Client::Resource::Resource and OC::Cm::Client::Resource::IResourceResponseListener @n
 ...
 */
class DetectionResource : public OC::Cm::Client::Resource::Resource
{

    public:

        /**
         * Constructor of DetectionResource class.
         *
         ...
         */
        DetectionResource(OC::Cm::IContext *context, std::string &protocol, std::string &deviceAddress);

        /**
         * Default virtual destructor of DetectionResource class.
         */
        virtual ~DetectionResource();
         ...
         *
         * @param[in]   DetectionResourceResponseListener   Object that implements IDetectionResourceResponseListener interface
         */

        void addResponseListener (IDetectionResourceResponseListener &respListener) ;

        IDetectionResourceResponseListener *getResponseListener ();

        bool getDetection( int &requestId );


        bool putDetection( int &requestId , ::Detection &reqData);


        void buildPath(std::string deviceId);

    private:
        class DetectionResourceListenerInternal : public OC::Cm::Client::Resource::IResourceResponseListener
        {
            public:

                DetectionResource *resource;

                DetectionResourceListenerInternal(DetectionResource *res)
                {
                    resource = res;
                }

                ~DetectionResourceListenerInternal()
                {
                }
                ...
                {
                    IDetectionResourceResponseListener *p_respListener = resource->getResponseListener();
                    if (p_respListener != NULL)
                    {
                        if (response != NULL)
                        {
                            ...
                            if ( 0 == elementName.compare(EN_DETECTION))
                            {
                                ::Detection *respData = (::Detection *)response ;
                                p_respListener->ongetDetection(clientSessionId, status,  respData) ;
                            }
                            else
                            {
                                p_respListener->ongetDetection(clientSessionId, 400,  NULL) ;
                            }
                        }
                        else
                        {
                            p_respListener->ongetDetection(clientSessionId, status,  NULL) ;
                        }
                                      ...
                                      OC::Cm::Serialization::ISerializable *response)
                {
                    ...
                    if (p_respListener != NULL)
                    {
                        p_respListener->onputDetection( clientSessionId, status ) ;
                    }
                }
                void
        ...
        /**
         * Variable that contains IDetectionResourceResponseListener object.
         */
        IDetectionResourceResponseListener *p_respListener ;

        OC::Cm::Client::Resource::IResourceResponseListener *p_internalResponseListener ;

};
/** @} */
#endif /* DETECTIONRESOURCE_H_ */
