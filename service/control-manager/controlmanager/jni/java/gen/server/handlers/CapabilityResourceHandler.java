 ...
 * CapabilityResourceHandler class.
 * 
 * @since 1.0.0
 */
public class CapabilityResourceHandler extends SyncResourceHandler {
    /**
     * Default constructor of CapabilityResourceHandler class.
     * 
     * @since 1.0.0
     */
    public CapabilityResourceHandler() {
    }
        ...
        if (-1 != allowed_methods.indexOf(methodId)) {
            if (methodId.equalsIgnoreCase("GET")) {
                if (false == validateRepresentation(response,
                        Elements.EN_CAPABILITY)) {
                    return 404;
                }
                statusCode = this.onGET(session, (gen.xsd.Capability) response);
     ...
     *            Server session to which the request belongs
     * @param respData
     *            Capability object to be returned to the server through the
    ...
    int onGET(OC.Cm.Connector.Server.ServerSession session,
            gen.xsd.Capability respData) {
        String contextName = OC.Cm.ControlManager.getContextName(session
                .getContext());
        gen.cm.MyCMDevice.getInstance(contextName).getCapability(respData);
        return 200;
    }
