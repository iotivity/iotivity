 ...
 * DeviceResourceHandler class.
 * 
 * @since 1.0.0
 */
public class DeviceResourceHandler extends SyncResourceHandler {
    /**
     * Default constructor of DeviceResourceHandler class.
     * 
     * @since 1.0.0
     */
    public DeviceResourceHandler() {
    }
    ...
    public int handleSyncResource(ServerSession session, String methodId,
        ...
        int statusCode = 400;
        String allowed_methods = "GET PUT DELETE";
        ...
        if (-1 != allowed_methods.indexOf(methodId)) {
            if (methodId.equalsIgnoreCase("GET")) {
                ...
                statusCode = this.onGET(session, (gen.xsd.Device) response);

            } else if (methodId.equalsIgnoreCase("PUT")) {
                if (false == validateRepresentation(request, Elements.EN_DEVICE)) {
                    return 404;
                }
                statusCode = this.onPUT(session, (gen.xsd.Device) request);
     ...
     *            Server session to which the request belongs
     * @param respData
     *            Device object to be returned to the server through the
     ...
     *            Server session to which the request belongs
     * @param reqData
     *            Received Device object through the deserializer. This object
