//******************************************************************
//
// Copyright 2014 Samsung Electronics All Rights Reserved.
//
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.
//
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

#include "UsageLimits.h"

#include "Elements.h"

using namespace std;

UsageLimits::UsageLimits()
{
    setList(true);

}

UsageLimits::~UsageLimits()
{
    for (std::list< ::UsageLimit *>::const_iterator itr = mpUsageLimit.begin();
         itr != mpUsageLimit.end(); itr++)
    {
        delete (*itr);
    }
    mpUsageLimit.clear();

}

bool UsageLimits::serializeContents(
    OC::Cm::Serialization::ISerializer &serializer)
{
    if (false == ::List::serializeContents(serializer))
    {
        return false;
    }

    {
        bool bRet = true;

        if (mpUsageLimit.size() > 0)
        {
            if (serializer.validateMandatory() == false)
            {
                serializer.setIDExcemption(true);
            }
            serializer.startListProperty("UsageLimits");
            for (std::list< ::UsageLimit *>::const_iterator itr =
                     mpUsageLimit.begin(); itr != mpUsageLimit.end(); itr++)
            {
                (*itr)->setInstanceName("UsageLimits");
                if (false == (*itr)->serialize(serializer))
                {
                    bRet = false;
                    break;
                }
            }
            serializer.endListProperty("UsageLimits");
            if (false == bRet)
            {
                if (serializer.isInFilteringMode("UsageLimits") == false)
                {
                    serializer.setErrorMessage(
                        "Failed To Serialize parameter (Invalid content) : UsageLimits");
                    return false;
                }
            }
        }

    }

    return true;
}

bool UsageLimits::deSerializeContents(
    OC::Cm::Serialization::IDeserializer &deserializer)
{
    if (false == ::List::deSerializeContents(deserializer))
    {
        return false;
    }

    {
        bool isArray = deserializer.getIsArray("UsageLimits");
        int sizeOfArray = 0;

        if (deserializer.validateMandatory() == false)
        {
            deserializer.setIDExcemption(true);
        }

        if (isArray)
        {
            sizeOfArray = deserializer.getSize("UsageLimits");

            if (sizeOfArray > 0)
            {
                for (int i = 0; i < sizeOfArray; i++)
                {
                    ::UsageLimit *m = new ::UsageLimit();

                    deserializer.setCurrentArrayIndex("UsageLimits", i);

                    m->setInstanceName("UsageLimits");

                    if (false == m->deSerialize(deserializer))
                    {
                        return false;
                    }

                    mpUsageLimit.push_back(m);
                }
            }
            else if (sizeOfArray == 0)
            {
                deserializer.removeParameter("UsageLimits");
            }
        }
        else
        {
            if (true == deserializer.hasProperty("UsageLimits"))
            {
                ::UsageLimit *m = new ::UsageLimit();

                m->deSerialize(deserializer);

                mpUsageLimit.push_back(m);
            }
            else
            {
                if ((deserializer.isInFilteringMode("UsageLimits") == false))
                {
                    deserializer.setErrorMessage(
                        "Failed To deserialize : UsageLimits");
                    return false;
                }
            }
        }
    }

    return true;
}

std::string UsageLimits::getElementName()
{
    return EN_USAGELIMITS;
}

bool UsageLimits::clone(::UsageLimits *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false;
    }

    {
        for (std::list< ::UsageLimit *>::const_iterator itr =
                 mpUsageLimit.begin(); itr != mpUsageLimit.end(); itr++)
        {
            delete (*itr);
        }
        mpUsageLimit.clear();
        for (std::list< ::UsageLimit *>::const_iterator itr =
                 pSrcObject->mpUsageLimit.begin();
             itr != pSrcObject->mpUsageLimit.end(); itr++)
        {
            ::UsageLimit *pTemp = new ::UsageLimit();
            if (false == pTemp->clone((*itr)))
            {
                delete pTemp;
                return false;
            };
            mpUsageLimit.push_back(pTemp);
        }
    }

    return true;
}
