...
Alarms::~Alarms()
{
    for (std::list< ::Alarm *>::const_iterator itr = mpAlarm.begin();
         itr != mpAlarm.end(); itr++)
    {
        delete (*itr);
    }
...
bool Alarms::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        if (mpAlarm.size() > 0)
        {
            ...
            if (false == bRet)
            {
                if (serializer.isInFilteringMode("Alarms") == false)
                {
                    serializer.setErrorMessage(
                        "Failed To Serialize parameter (Invalid content) : Alarms");
                    return false;
                }
            }
        }
...
bool Alarms::deSerializeContents(
    OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Alarms") == false))
                {
...
std::string Alarms::getElementName()
{
    return EN_ALARMS;
}

bool Alarms::clone(::Alarms *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false;
...

    {
        ...
        mpAlarm.clear();
        for (std::list< ::Alarm *>::const_iterator itr =
                 pSrcObject->mpAlarm.begin(); itr != pSrcObject->mpAlarm.end();
             itr++)
        {
