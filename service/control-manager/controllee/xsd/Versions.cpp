...
Versions::~Versions()
{
    for (std::list< ::Version *>::const_iterator itr = mpVersion.begin();
         itr != mpVersion.end(); itr++)
    {
        delete (*itr);
    }
    mpVersion.clear();
...
bool Versions::serializeContents(
    OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        if (mpVersion.size() > 0)
        {
            ...
            if (false == bRet)
            {
                if (serializer.isInFilteringMode("Versions") == false)
                {
                    serializer.setErrorMessage(
                        "Failed To Serialize parameter (Invalid content) : Versions");
                    return false;
                }
            }
        }
...
bool Versions::deSerializeContents(
    OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Versions") == false))
                {
                    deserializer.setErrorMessage(
                        "Failed To deserialize : Versions");
                    return false;
                }
            }
        }
...
std::string Versions::getElementName()
{
    return EN_VERSIONS;
}

bool Versions::clone(::Versions *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false;
...

    {
        ...
        mpVersion.clear();
        for (std::list< ::Version *>::const_iterator itr =
                 pSrcObject->mpVersion.begin();
