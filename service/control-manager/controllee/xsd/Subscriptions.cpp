...
Subscriptions::~Subscriptions()
{
    for (std::list< ::Subscription *>::const_iterator itr =
             mpSubscription.begin(); itr != mpSubscription.end(); itr++)
    {
        delete (*itr);
    }
...
bool Subscriptions::serializeContents(
    OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        if (mpSubscription.size() > 0)
        {
            ...
            for (std::list< ::Subscription *>::const_iterator itr =
                     mpSubscription.begin(); itr != mpSubscription.end();
                 itr++)
            {
            ...
            if (false == bRet)
            {
                if (serializer.isInFilteringMode("Subscriptions") == false)
                {
                    serializer.setErrorMessage(
                        "Failed To Serialize parameter (Invalid content) : Subscriptions");
                    return false;
                }
            }
        }
...
bool Subscriptions::deSerializeContents(
    OC::Cm::Serialization::IDeserializer &deserializer)
{
...

    {
        ...
        else
        {
            ...
            {
                if ((deserializer.isInFilteringMode("Subscriptions") == false))
                {
                    deserializer.setErrorMessage(
                        "Failed To deserialize : Subscriptions");
                    return false;
                }
            }
        }
...
std::string Subscriptions::getElementName()
{
    return EN_SUBSCRIPTIONS;
}

bool Subscriptions::clone(::Subscriptions *pSrcObject)
{
    if (pSrcObject == NULL)
    {
        return false;
...

    {
        ...
        mpSubscription.clear();
        for (std::list< ::Subscription *>::const_iterator itr =
                 pSrcObject->mpSubscription.begin();
