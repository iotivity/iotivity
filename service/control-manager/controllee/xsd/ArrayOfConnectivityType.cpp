...
ArrayOfConnectivityType::ArrayOfConnectivityType()
{
...
bool
ArrayOfConnectivityType::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
    bool bRet = true ;

    serializer.startListProperty(this->getInstanceName()) ;

    for ( std::list< ::ConnectivityType >::iterator itr = members.begin(); itr != members.end(); itr++)
    {
...
bool
ArrayOfConnectivityType::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
    bool bRet = true ;
...
std::string
ArrayOfConnectivityType::getElementName()
{
    return "XS:LIST";
}
