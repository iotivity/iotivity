...
Devices::~Devices()
{
    for ( std::list< ::Device * >::const_iterator itr = mpDevice.begin(); itr != mpDevice.end(); itr++ )
    {
        delete (*itr);
    }
    mpDevice.clear();
...
Devices::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
...

    {
        ...
        serializer.startListProperty("Devices") ; for ( std::list< ::Device * >::const_iterator itr = mpDevice.begin(); itr != mpDevice.end(); itr++ )
        {
            (*itr)->setInstanceName("Devices") ;
            if (false == (*itr)->serialize(serializer)) { bRet = false; break; }
        }
        serializer.endListProperty("Devices") ; if (false == bRet) { if (serializer.isInFilteringMode("Devices") == false) {serializer.setErrorMessage("Failed To Serialize parameter (Invalid content) : Devices"); return false; }}
    }
...
Devices::clone(::Devices *pSrcObject)
{
...

    {
        ...
        mpDevice.clear();
        for ( std::list< ::Device * >::const_iterator itr = pSrcObject->mpDevice.begin(); itr != pSrcObject->mpDevice.end(); itr++ )
        {
