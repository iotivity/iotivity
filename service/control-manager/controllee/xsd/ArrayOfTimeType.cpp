...
#include "ArrayOfTimeType.h"
#include "Elements.h"

using namespace std;

ArrayOfTimeType::ArrayOfTimeType()
{
...
bool
ArrayOfTimeType::serializeContents(OC::Cm::Serialization::ISerializer &serializer)
{
    bool bRet = true ;

    serializer.startListProperty(this->getInstanceName()) ;

    for ( std::list< OC::Cm::Serialization::Xsd::TimeType >::iterator itr = members.begin(); itr != members.end(); itr++)
    {
...
bool
ArrayOfTimeType::deSerializeContents(OC::Cm::Serialization::IDeserializer &deserializer)
{
    bool bRet = true ;
    ...
    if (isArray)
    {
        ...
        for (int i = 0; i < sizeOfArray; i++)
        {
            ...
            OC::Cm::Serialization::Xsd::TimeType el ;
...
std::string
ArrayOfTimeType::getElementName()
{
    return "XS:LIST";
}
