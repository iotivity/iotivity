...
#include "TemperatureResourceHandler.h"

TemperatureResourceHandler::TemperatureResourceHandler(
...
bool TemperatureResourceHandler::handleSyncResource(std::string methodId,
    ...
    if (0 == methodId.compare("GET"))
    {
        if (false
            == OC::Cm::Server::ResourceHandler::SyncResourceHandler::validateRepresentation(
                response, EN_TEMPERATURE))
        {
            statusCode = 404;
            return true;
        }
        this->onGET(statusCode, (::Temperature *) response);
    }
    else if (0 == methodId.compare("PUT"))
    {
        if (false
            == OC::Cm::Server::ResourceHandler::SyncResourceHandler::validateRepresentation(
                request, EN_TEMPERATURE))
        {
            statusCode = 404;
            return true;
        }
        this->onPUT(statusCode, (::Temperature *) request);
    }
    else
    {
        statusCode = 405;
...
bool TemperatureResourceHandler::onGET(int &statusCode,
...
bool TemperatureResourceHandler::onPUT(int &statusCode,
                                       ::Temperature *reqData)
{
    // TODO: Autogenerated code. Add Resource implementation here
    // TODO: Default Status Code is: 501 - Not Implemented.  Replace Default Status Code based on implementation!
