...
bool InformationResourceHandler::handleSyncResource(std::string methodId,
    ...
    if (0 == methodId.compare("GET"))
    {
        if (false
            == OC::Cm::Server::ResourceHandler::SyncResourceHandler::validateRepresentation(
                response, EN_INFORMATION))
        {
            statusCode = 404;
            return true;
        }
        this->onGET(statusCode, (::Information *) response);
    }
    else
    {
        statusCode = 405;
...
bool InformationResourceHandler::onGET(int &statusCode,
