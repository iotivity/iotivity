...
#include "TemperaturesResourceHandler.h"

TemperaturesResourceHandler::TemperaturesResourceHandler(
...
bool TemperaturesResourceHandler::handleSyncResource(std::string methodId,
    ...
    if (0 == methodId.compare("GET"))
    {
        if (false
            == OC::Cm::Server::ResourceHandler::SyncResourceHandler::validateRepresentation(
                response, EN_TEMPERATURES))
        {
            statusCode = 404;
            return true;
        }
        this->onGET(statusCode, (::Temperatures *) response);
    }
    else if (0 == methodId.compare("PUT"))
    {
        if (false
            == OC::Cm::Server::ResourceHandler::SyncResourceHandler::validateRepresentation(
                request, EN_TEMPERATURES))
        {
            statusCode = 404;
            return true;
        }
        this->onPUT(statusCode, (::Temperatures *) request);
    }
    else
    {
        statusCode = 405;
...
bool TemperaturesResourceHandler::onGET(int &statusCode,
...
bool TemperaturesResourceHandler::onPUT(int &statusCode,
                                        ::Temperatures *reqData)
{
    // TODO: Autogenerated code. Add Resource implementation here
    // TODO: Default Status Code is: 501 - Not Implemented.  Replace Default Status Code based on implementation!
    statusCode = 501;
