...
#include <OutsideResourceServer.h>
...
class Temphumid
{
    public:
        ...
        Temphumid() : m_temp(0), m_humid(0)//, m_name("")
        {
        }
};
                    ...
                    const int& eCode, const int& sequenceNumber)
{
    try
    {
        if(eCode == OC_STACK_OK && sequenceNumber != OC_OBSERVE_NO_OPTION)
        {
            ...
            rep.getValue("oic.r.temperature", mylight.m_temp);
            rep.getValue("oic.r.humidity", mylight.m_humid);
            // rep.getValue("name", mylight.m_name);

            std::cout << "\ttemperature: " << mylight.m_temp << std::endl;
            std::cout << "\thumidity: " << mylight.m_humid << std::endl;
        ...
        const OCRepresentation& rep, const int eCode)
{
    try
    {
        if(eCode == OC_STACK_OK || eCode == OC_STACK_RESOURCE_CREATED)
        {
            ...
            rep2.setValue("oic.r.temperature", mylight.m_temp);
            rep2.setValue("oic.r.humidity", mylight.m_humid);
...
void postLightRepresentation(std::shared_ptr<OCResource> resource)
{
    if(resource)
    {
        ...
        rep.setValue("oic.r.temperature", mylight.m_temp);
        rep.setValue("oic.r.humidity", mylight.m_humid);
...
void onGet(const HeaderOptions& /*headerOptions*/, const OCRepresentation& rep, const int eCode)
{
    try
    {
        if(eCode == OC_STACK_OK)
        {
            ...
            // rep.getValue("temperature", mylight.m_temp);
            // rep.getValue("humidity", mylight.m_humid);
            // rep.getValue("name", mylight.m_name);
            rep.getValue("oic.r.temperature", mylight.m_temp);
            rep.getValue("oic.r.humidity", mylight.m_humid);
            // rep.getValue("name", mylight.m_name);
...
void foundResource(std::shared_ptr<OCResource> resource)
{
    ...
    try
    {
        ...
        if(resource)
        {
            ...
            if(resourceURI == "/Thing_TempHumSensor")
            {
