...
void MoistResource::registerResource()
{
    ...
    {
        cout << "MoistResource creation was unsuccessful\n";
    }
    else
    {
        cout << "MoistResource creation is successful" << endl;
    }
}

OCResourceHandle MoistResource::getHandle()
{
    return m_resourceHandle;
}

void MoistResource::setResourceRepresentation(OCRepresentation &rep)
{
    double tempMoist;

    rep.getValue("moist", tempMoist);

    m_moist = tempMoist;

    cout << "\tReceived representation: " << endl;

    cout << "\tmoist: " << m_moist << endl;

}

OCRepresentation MoistResource::getResourceRepresentation()
{
    m_resourceRep.setValue("moist", std::to_string(m_moist));
    cout << "Moist is set to " << m_moist << endl;

    return m_resourceRep;
}

MoistResource g_myResource;
...
void *TestSensorVal(void *param)
{
    ...
    std::cout << "[MoistSensorAPP] ::" << __func__ << " is called."
              << std::endl;
    ...
    while (1)
    {
        ...
        if (g_Observation)
        {
            ...
            cout << "\n moist updated to : " << g_myResource.m_moist << endl;
...
OCEntityHandlerResult entityHandler(std::shared_ptr< OCResourceRequest > request)
{
    cout << "\tIn Moist Server CPP entity handler:\n";
