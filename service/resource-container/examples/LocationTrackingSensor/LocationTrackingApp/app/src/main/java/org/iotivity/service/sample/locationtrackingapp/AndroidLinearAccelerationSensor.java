...
public class AndroidLinearAccelerationSensor extends AndroidBundleResource implements SensorEventListener {
    ...
    public AndroidLinearAccelerationSensor(Context appContext) {
        super(appContext);
        this.setResourceType("oic.r.linearacceleration");
        this.setName("androidLinearAcceleration");

        mSensorManager = (SensorManager) appContext.getApplicationContext().getSystemService(Context.SENSOR_SERVICE);
        linearaccelSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_LINEAR_ACCELERATION);
        mSensorManager.registerListener(this, linearaccelSensor, SensorManager.SENSOR_DELAY_NORMAL);
    }
    ...
    public void onSensorChanged(SensorEvent event) {
        setAttribute("linearaccel_x", new RcsValue(event.values[0]), true);
        setAttribute("linearaccel_y", new RcsValue(event.values[1]), true);
        setAttribute("linearaccel_z", new RcsValue(event.values[2]), true);
    }
