...
void ActivityResource::registerResource()
{
...
OCResourceHandle ActivityResource::getHandle()
{
    return m_resourceHandle;
}

void ActivityResource::setResourceRepresentation(OCRepresentation &rep)
{
    double tempActivity;

    rep.getValue("heartrate", tempActivity);

    heartrate = tempActivity;
...
OCRepresentation ActivityResource::getResourceRepresentation()
{
    m_resourceRep.setValue("heartrate", std::to_string(heartrate));

    return m_resourceRep;
}

// Create the instance of the TemphumidResource class
ActivityResource g_myResource;
...
void *TestSensorVal(void *param)
{
    ...
    std::cout << "[ActivitySensorAPP] ::" << __func__ << " is called."
              << std::endl;
    ...
    while (1)
    {
        ...
        if (g_Observation)
        {
            if (bFlag)
            {
                ...
                nActivity += DIFF_VAL;
                g_myResource.heartrate = nActivity;
            }
            else
            {
            ...
            cout << "\n heart rate updated to : " << g_myResource.heartrate << endl;
