...
import oic.ctt.devicecontroller.SystemUtil;
...
public class LinuxProcess implements IProcess {
    ...
    public LinuxProcess(String processName, String cmd) {
        try {
            ...
            stdintBufferedWriter = new BufferedWriter(new OutputStreamWriter(
                    consoleProcess.getOutputStream()));
    ...
    public boolean destroy() {
        try {
            ...
            logger.info("Exception occured in destroying process "
                    + e.toString());
            e.printStackTrace();
    ...
    public class ProcessLogObservable implements ILogObservable {
        ...
        public ProcessLogObservable() {
            ...
            logger.info("ProcessLogObservable");
        }
                        ...
                        for (;;) {
                            try {
                                ...
                                if (inputLine != null) {
                                    ...
                                    logger.info("Breaking reading line from input stream");
                                    break;
                                }
                            } catch (Exception e) {
                                logger.info("Exception occured " + e.toString());
