...
public class OICClient {
     ...
     */
    public ArrayList<OICResponseData> discoverResource(
            OICClient.Protocol protocol, String uriPath) {
     ...
     */
    public ArrayList<OICResponseData> discoverResource(
            OICClient.Protocol protocol, String uriPath, String query) {
     ...
     */
    public OICResponseData sendRequest(OICClient.Protocol protocol,
     ...
     *            Port of the recipient
     */
    public void observeResource(OICClient.Protocol protocol,
            OICHelper.MessageType type, String messageId, String token,
            String ip, int port) {
        observeResource(protocol, type, messageId, token, ip, port,
                OICHelper.getDefaultUri());
    }
    ...
    public void observeResource(OICClient.Protocol protocol,
        ...
        observeResource(protocol, type, messageId, token, ip, port, uriPath,
                OICHelper.DEFAULT_QUERY);
    }
    ...
    public void observeResource(OICClient.Protocol protocol,
        ...
        mClientMap.get(protocol)
                .observeResource(type, Integer.parseInt(messageId),
                        OICHelper.hexStringToByteArray(token), ip, port,
                        uriPath, query);
    }
    ...
    public ArrayList<OICResponseData> getNotifications(
        ...
        return mClientMap.get(protocol).getNotifications();
    }
     ...
     *            The protocol being used (HTTP or CoAP)
     */
    public void cancelObservePassively(OICClient.Protocol protocol) {

        if (protocol == null)
            protocol = OICClient.Protocol.COAP;

        mClientMap.get(protocol).cancelObservePassively();
    }
     ...
     *            The protocol being used (HTTP or CoAP)
     */
    public void cancelObserveWithReset(OICClient.Protocol protocol) {

        if (protocol == null)
            protocol = OICClient.Protocol.COAP;

        mClientMap.get(protocol).cancelObserveWithReset();
    }
    ...
    public OICResponseData cancelObserveWithGetMessage(
            OICHelper.MessageType type, String messageId, String ip, int port,
            String uriPath) {
        return cancelObserveWithGetMessage(type, messageId, ip, port, uriPath,
                OICHelper.DEFAULT_QUERY);
    }
    ...
    public OICResponseData cancelObserveWithGetMessage(
            OICHelper.MessageType type, String messageId, String ip, int port,
            String uriPath, String query) {
        return mCoapClient.cancelObserveWithGetMessage(type,
                Integer.parseInt(messageId), ip, port, uriPath, query);
    }
}
