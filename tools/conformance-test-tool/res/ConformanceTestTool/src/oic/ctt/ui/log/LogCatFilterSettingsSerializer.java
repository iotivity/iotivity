 ...
 *
 * Users can create multiple filters in the logcat view. These filters could
 * have regexes in their settings. All of the user created filters are saved
 * into a single Eclipse preference. This class helps in generating the string
 * to be saved given a list of {@link LogCatFilter}'s, and also does the reverse
 * of creating the list of filters given the encoded string.
 */
public final class LogCatFilterSettingsSerializer {
    ...
    /**
     * Encode the settings from a list of {@link LogCatFilter}'s into a string
     * for saving to the preference store. See
     * {@link LogCatFilterSettingsSerializer#decodeFromPreferenceString(String)}
     * for the reverse operation.
     * 
     * @param filters
     *            list of filters to save.
     * @return an encoded string that can be saved in Eclipse preference store.
     *         The encoded string is of a list of key:'value' pairs.
     */
    public String encodeToPreferenceString(List<LogFilter> filters) {
        ...
        for (LogFilter f : filters) {
            ...
            sb.append(KW_TEXT);
            sb.append(KW_DELIM);
            sb.append(quoteString(f.getText()));
            sb.append(ATTR_DELIM);

            sb.append(KW_TAG);
            sb.append(KW_DELIM);
            sb.append(quoteString(f.getTag()));
            sb.append(ATTR_DELIM);

            sb.append(KW_ROBOTSYSLOG);
            sb.append(KW_DELIM);
            sb.append(quoteString(f.getRobotSysLog()));
            sb.append(ATTR_DELIM);
     ...
     */
    public List<LogFilter> decodeFromPreferenceString(String pref) {
        List<LogFilter> fs = new ArrayList<LogFilter>();
        ...
        while (index < kv.size()) {
            ...
            while (index < kv.size() && !kv.get(index).equals(KW_NAME)) {
                ...
                } else if (key.equals(KW_TAG)) {
                    tag = value;
                } else if (key.equals(KW_ROBOTSYSLOG)) {
                    robot = value;
                }
            }
            fs.add(new LogFilter(name, tag, text, robot, level));
        }
        return fs;
    }
    ...
    private String quoteString(String s) {
        return SINGLE_QUOTE
                + s.replace(Character.toString(SINGLE_QUOTE), "\\'")
                + SINGLE_QUOTE;
    }
    ...
    private String getNextString(String s) {
        ...
        while (index < s.length()) {
            ...
            if (index > 0 && s.charAt(index) == SINGLE_QUOTE
                    && s.charAt(index - 1) != ESCAPE_CHAR) {
