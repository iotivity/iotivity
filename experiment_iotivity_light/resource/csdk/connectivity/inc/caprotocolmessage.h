...
typedef uint32_t code_t; // code can be either method or error code
 ...
 * @return  CA_STATUS_OK or ERROR CODES (CAResult_t error codes in cacommon.h).
 */
CAResult_t CAParseURI(const char *uriInfo, int target, coap_list_t **options);
 ...
 * @return  CA_STATUS_OK or ERROR CODES (CAResult_t error codes in cacommon.h).
 */
CAResult_t CAGetInfoFromPDU(const coap_pdu_t *pdu, CAMessageInfo_t *messageInfo);
 ...
 * @return  CA_STATUS_OK or ERROR CODES (CAResult_t error codes in cacommon.h).
 */
CAResult_t CAGenerateTokenInternal(OCToken_t *token, uint8_t tokenLength);
 ...
 * @return  message type.
 */
OCMessageType_t CAGetMessageTypeFromPduBinaryData(const void *pdu, uint32_t size);
