...
static uint64_t AsciiHexToValue(char* hexString, int length);
...
TWResultCode TelNetworkInfoHandler(int count, char* tokens[])
{
    ...
    else
    {
        ...
        g_ZigBeeStatus.panId = (int)AsciiHexToValue(tokens[3], 2);
        g_ZigBeeStatus.extPanId = AsciiHexToValue(tokens[4], 8);
...
TWResultCode TelJpanHandler(int count, char* tokens[])
{
    ...
    g_ZigBeeStatus.panId = AsciiHexToValue(tokens[1], 2);
    g_ZigBeeStatus.extPanId = AsciiHexToValue(tokens[2], 8);
...
uint64_t AsciiHexToValue(char* hexString, int length)
{
    uint64_t value = AsciiToHex(hexString[0]);
    int i;
    for (i = 1; i < length*2; ++i)
    {
        ...
        {
            value <<= 4;
            value |= (uint64_t) AsciiToHex(hexString[i]);
        }
    }
    return value;
}
...
bool IsThisAValidLine(char * singleLine, bool * endControlHit, uint64_t * atErrorCode)
{
    ...
    for(; i<TW_ENDCONTROL_MAX_VALUE; i++)
    {
        ...
        if ((singleLine) &&
            (strncmp(singleLine, g_TWEndMessageMap[i].endStr, strSize) == 0))
        {
            //endControlHit = true;
            ...
            {
                *atErrorCode = AsciiHexToValue(&singleLine[strSize],
...
char * myStrTok(char* readData, const char* tokens, bool * endControlHit, uint64_t * atErrorCode)
{
    char * buf = strtok(readData, tokens);
    if(!buf)
    {
        return NULL;
    }
    ...
    {
        return myStrTok(NULL, tokens, endControlHit, atErrorCode);
    }
}
                        ...
                        uint8_t timeout)
{
    ...
    while(itrDataBytes == 0)
    {
        ...
        if(command)
        {
            char * sendCommand = OICStrdup(command);
            if (!sendCommand)
            {
                return TW_RESULT_ERROR_NO_MEMORY;
            }
            sendCommand = (char *) realloc(sendCommand, strlen(sendCommand) + strlen("\r") + 1);
            if (!sendCommand)
            {
                return TW_RESULT_ERROR_NO_MEMORY;
            }
            OICStrcat(sendCommand, sizeof(sendCommand), "\r");
            if(strcmp(itrData, sendCommand) == 0)
            {
