...
public class CANetworkCommonTest extends AndroidTestCase {
	...
	private static final int NETWORK = 4; // For Edr=4, Le=2
	...
	public void testCallOtherCAApiWithoutCallingCAInitialize_N() {
		...
		if (MainActivity.RM
				.startListeningServerCustom(CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}

		if (MainActivity.RM.stopListeningServer(CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}

		if (MainActivity.RM
				.startDiscoveryWithExpectedResult(CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}

		if (MainActivity.RM.selectNetworkWithExpectedResult(NETWORK,
				CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}

		if (MainActivity.RM.unSelectNetworkWithExpectedResult(NETWORK,
				CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}

		if (MainActivity.RM.getNetworkInfomationWithExpectedResult(
				Get_NETWORK_INFO_WITHOUT_INIT, CA_STATUS_NOT_INITIALIZED)) {
			fail();
		}
	}
	...
	public void testCAInitializeAndCATerminateLoopWithOtherFn_P() {
		for (int count = 0; count < TRY_COUNT; count++) {
			...
			if (!MainActivity.RM.unSelectNetwork()) {
				fail();
			}
	...
	public void testCASelectNetworkUnSelectNetwork_EG_N() {
		...
		if (!MainActivity.RM.unSelectNetworkWithExpectedResult(0,
				CA_STATUS_FAILED)) {
			fail();
			return;
		}
	...
	public void testCACreateEndpointDestroyEndpoint_VLCC_P() {
		...
		for (int count = 0; count < TRY_COUNT; count++) {
			...
			if (!MainActivity.RM.destroyEndpoint()) {
				fail();
			}
