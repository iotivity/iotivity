...
public class ICMessageQueueTest extends AndroidTestCase {
    ...
    OnMQTopicCreatedListener    m_onTopicCreatedListener = new OnMQTopicCreatedListener() {
                                                             public void onTopicResourceCreated(
                                                                     OcResource m_myResource) {
                                                             }

                                                             public void onCreateTopicFailed(
                                                                     Throwable ex,
                                                                     String m_uri) {
                                                             }
                                                         };
    OnMQTopicFoundListener      m_onTopicFoundListener   = new OnMQTopicFoundListener() {
                                                             public void onTopicDiscoveried(
                                                                     OcResource m_myResource) {
                                                             }

                                                             public void onDiscoveryTopicFailed(
                                                                     Throwable ex,
                                                                     String m_uri) {
                                                             }
                                                         };
    OnPostListener              m_onPostListener         = new OnPostListener() {
                                                             public void onPostCompleted(
                                                                     List<OcHeaderOption> m_headerOptionList,
    ...
    public void testrequestMQPublish_ESV_ETC_N() {
        try {
            m_ocResource.requestMQPublish(null, m_onPostListener,
                    m_qualityOfService);
            fail(SHOULD_THROW_EXCEPTION);
        } catch (OcException ex) {
            assertTrue(true);
     ...
     * @target public void unsubscribeMQTopic(QualityOfService qualityOfService)
     * @test_data 1. host Host address 2. unsubscribeMQTopic
     * @pre_condition none
