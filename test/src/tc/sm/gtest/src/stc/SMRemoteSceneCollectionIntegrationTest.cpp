 ...
 * 					2. check onRemoteSceneCreated is invoked
...
TEST_F(SMRemoteSceneCollectionIntegrationTest_stc, RemoteAddNewScene_CV_P)
{
	...
	try
	{
		m_pRemoteSceneCollection->addNewScene(sceneName1,std::bind(&SMRemoteSceneCollectionIntegrationTest_stc::onRemoteSceneCreated, this,placeholders::_1, placeholders::_2));
		CommonUtil::waitInSecond(CALLBACK_WAIT_MIN);
	}
	catch (exception& e)
	{
	...
	if(!isCallback)
	{
		SET_FAILURE("AddNewSceneCallback is not invoked");
	}
 ...
 *	 				4. cb A callback to receive created RemoteScene instance
 * @pre_condition 	1. Start SceneList type resource using SCENE_SERVER simulator
 * 					2. discover resource
 * 					3. get remote SceneList instance
 * 					4. get remote SceneCollection pointer by adding SceneCollection
...
TEST_F(SMRemoteSceneCollectionIntegrationTest_stc, RemoteAddAndGetScenes_GSRV_P)
{
	...
	try
	{
		...
		m_pRemoteSceneCollection->addNewScene(sceneName2,std::bind(&SMRemoteSceneCollectionIntegrationTest_stc::onRemoteSceneCreated, this,placeholders::_1, placeholders::_2));
		CommonUtil::waitInSecond(CALLBACK_WAIT_MIN);
		...
		ASSERT_EQ(m_pRemoteScene1,remoteScenes.at(sceneName1)) << "first received scene did not match the firstly added scene";
		ASSERT_EQ(m_pRemoteScene2,remoteScenes.at(sceneName2)) << "second received scene did not match the secondly added scene";
	}
	catch (exception& e)
	{
		SET_FAILURE("Exception occurred: " + std::string(e.what()));
 ...
 * @procedure 		1. call addNewScene to add a new scene
 * 					2. call setName
 * 					3. check onRemoteSceneCreated callback is invoked
 * @post_condition stop the simulator
 * @expected callback should be called and no exceptions occur
 */
#if defined(__LINUX__)
TEST_F(SMRemoteSceneCollectionIntegrationTest_stc, RemoteSceneCollectionSetName_CV_P)
{
	...
	try
	{
		m_pRemoteSceneCollection->setName(sceneCollectionName,std::bind(&SMRemoteSceneCollectionIntegrationTest_stc::onSetName, this,placeholders::_1));
		CommonUtil::waitInSecond(CALLBACK_WAIT_MIN);
	}
	catch (exception& e)
	{
	...
	if(!isCallback)
	{
		SET_FAILURE("SetNameCallback is not invoked");
	}
...
TEST_F(SMRemoteSceneCollectionIntegrationTest_stc, RemoteSceneCollectionSetAndGetName_GSRV_P)
{
	...
	try
	{
		...
		ASSERT_EQ(receivedName,sceneCollectionName) << "received name and set name did not match";
	}
	catch (exception& e)
	{
		SET_FAILURE("Exception occurred: " + std::string(e.what()));
