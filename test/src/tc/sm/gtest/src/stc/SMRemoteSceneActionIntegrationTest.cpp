...
public:
	...
	virtual void SetUp() {
		...
		ASSERT_NE(m_pRemoteSceneCollection,NULL_PTR)<< "remote scene action was not created";
 ...
 *             	  	2. value = "on"
 *             	  	3. cb A callback to receive the response
 ...
 * 					2. Check onActionUpdated is invoked or not.
 * @post_condition stop the simulators
 * @expected callback will be called and no exceptions should occur
 */
...
TEST_F(SMRemoteSceneActionIntregrationTest_stc, RemoteResetExecutionParameterWithKeyValue_CV_P)
{
	...
	if(!isCallback)
	{
		SET_FAILURE("ResetExecutionParameterCallback is not invoked");
	}
 ...
 * @test_data 		1. attr attribute to be set
 ...
 *                   const RCSResourceAttributes &attr, ResetExecutionParameterCallback cb)
 * @target RCSResourceAttributes getExecutionParameter() const;
 * @test_data 		1. attr attributes to be set
 *             	  	2. cb A callback to receive the response
 * @pre_condition 	1. Start SceneList and light type resource using SCENE_SERVER and LIGHT_SERVER simulators
 * 					2. get remote SceneList instance
 * 					3. get remote SceneCollection pointer by adding SceneCollection
 ...
 * @test_data 		1. targetResource A pointer of the discovered resource
 * 					2. key = "power"
 * 					3. value = "off"
 * 					4. cb callback to be called when remote scene action is created
 * @pre_condition 	1. Start SceneList and light type resource using SCENE_SERVER and LIGHT_SERVER simulators
 * 					2. get remote SceneList instance
 * 					3. get remote SceneCollection pointer by adding SceneCollection
 * 					4. Get scene pointer by adding scene
