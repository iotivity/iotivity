...
public class RIServerTest extends IoTivityTc {
    ...
    public void testConfigureServerOutOfProc_SRC_P() {
        try {
            PlatformConfig cfg = new PlatformConfig(ServiceType.OUT_OF_PROC,
    ...
    public void testConfigureServerNon_SRC_P() {
        try {
            PlatformConfig cfg = new PlatformConfig(ServiceType.IN_PROC,
                    ModeType.SERVER, "0.0.0.0", 0, QualityOfService.LOW);
            OcPlatform.Configure(cfg);
        } catch (Exception e) {
            e.printStackTrace();
            fail("Exception occured");
