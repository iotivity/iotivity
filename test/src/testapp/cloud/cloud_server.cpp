...
void initServer()
{
    ...
    {
        IOTIVITYTEST_LOG(ERROR, "[CLOUD SIMULATOR] OCRegisterPersistentStorageHandler error");
        return;
    }
...
static void createAclCb(void* g_ctx, OCStackResult result, void* data)
{
    ...
    IOTIVITYTEST_LOG(INFO, "%s: Received result = %d for g_ctx : %s", __func__, result,
            (char* )g_ctx);
...
static void getAclIdCb(void* g_ctx, OCStackResult result, void* data)
{
    OC_UNUSED(g_ctx);
    OC_UNUSED(data);
...
void createAcl()
{
    ...
    OCStackResult result = OCCloudAclIdCreate(g_ctx, DEFAULT_OWNER_ID, DEFAULT_DEV_ID, &g_endPoint,
            createAclCb);
    IOTIVITYTEST_LOG(INFO, "[Cloud Server] OCCloudAclIdCreate returns %s", CommonUtil::getOCStackResult(result));

    if (OC_STACK_OK != result)
    {
        IOTIVITYTEST_LOG(ERROR, "[Cloud Server] OCCloudAclIdCreate returns %s",
    ...
    if (OC_STACK_OK == result)
    {
        ...
        {
            IOTIVITYTEST_LOG(ERROR, "[Cloud Server] Callback Not Invoked");
            return;
        }
    }
    __FUNC_OUT__
...
void getAclId()
{
    __FUNC_IN__
    g_isCbInvoked = CALLBACK_NOT_INVOKED;
    OCStackResult result = OCCloudGetAclIdByDevice(g_ctx, DEFAULT_DEV_ID, &g_endPoint, getAclIdCb);
    IOTIVITYTEST_LOG(INFO, "[Cloud Server] OCCloudGetAclIdByDevice returns %s",
            CommonUtil::getOCStackResult(result));
    ...
    if (OC_STACK_OK == result)
    {
        if (CALLBACK_NOT_INVOKED == waitCallbackRet())
        {
            IOTIVITYTEST_LOG(ERROR, "[Cloud Server] ERROR");
            return;
        }
    }
    __FUNC_OUT__
