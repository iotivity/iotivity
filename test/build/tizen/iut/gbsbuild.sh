echo "Creating Temporary Directory Conformance Simulator"
...
tmpDir='tmp_cs'
target_arch='armv7l'
rpmName="com-oic-cs-0.0.1-1.armv7l.rpm"
...
cp com.oic.cs.spec $tmpDir/packaging
cp com.oic.cs.manifest $tmpDir
cp com.oic.cs.xml $tmpDir
...
gbscommand="gbs build -A "$target_arch" -B ~/GBS-ROOT-OIC --include-all --define 'TARGET_TRANSPORT IP' --define 'SECURED 1'"
echo $gbscommand
...
if eval $gbscommand; then
   ...
   mkdir -p ../$targetDir
   cp usr/apps/com.oic.cs/bin/* ../$targetDir
...
if [[ "${push}" = "true" ]] || [[ "${push}" = "1" ]]; then
    ...
    sdb $deviceId push $iotivity_path/resource/csdk/security/provisioning/sample/oic_svr_db_server_justworks.dat /usr/apps/com.oic.cs/bin/oic_svr_db_server_justworks.dat
    sdb $deviceId push $iotivity_path/resource/csdk/security/provisioning/sample/oic_svr_db_server_randompin.dat /usr/apps/com.oic.cs/bin/oic_svr_db_server_randompin.dat
    sdb $deviceId push $iotivity_path/resource/csdk/security/provisioning/sample/$db_filename /usr/apps/com.oic.cs/bin/oic_svr_db_server.dat
    
fi

mkdir -p $test_project_root/bin/tizen/iut
mv $test_project_root/build/tizen/iut/$rpmName $test_project_root/bin/tizen/iut/$rpmName
mv $test_project_root/build/tizen/iut/ConformanceSimulator $test_project_root/bin/tizen/iut/ConformanceSimulator
