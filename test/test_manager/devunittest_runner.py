...
oparser.set_defaults(target_source = 'target', platform='linux', bit='64', mode = 'release')
...
print("Runner Target Source : "+TARGET_SOURCE)
print("Runner Build Mode : "+BUILD_MODE)
print("Runner BIT : "+BIT)
START_STRING = ""
END_STRING = "============================================================================="
#set dev test result file directory
DEV_UNIT_TEST_RESULT_DIRECTORY = "../IotivityOrgSource/%s/iotivity/out/linux/%s/%s/test_out/"% (TARGET_SOURCE, ARCH, BUILD_MODE)
DEV_UNIT_TEST_RUN_DIRECTORY = "../IotivityOrgSource/%s/iotivity"% (TARGET_SOURCE)
...
def addCoverageFlag() :
    ...
    flag_add_command = "./add_gcov_flag.sh %s"%TARGET_SOURCE
...
def generateCoverageReport() :
    ...
    flag_add_command = "./generate_gcov_report_n_clean.sh %s"%TARGET_SOURCE
    execute_command(flag_add_command)
...
def build_secured_module() :
    ...
    #secured_run_command = "scons TARGET_OS=linux DTLS_WITH_X509=1 SECURED=1 TEST=1 -i -k"
...
def performQualityMeasure() :
    addCoverageFlag()
    Utility.change_directory(DEV_UNIT_TEST_RUN_DIRECTORY)
    downloadtinycbor()
    downloadMbedtls()
    ...
    Utility.change_directory(RUNNER_HOME_DIRECTORY)
...
def remove_secured_modules_xml() :
    ...
    for module_name in module :
         for file_name in module[module_name] :
          Utility.remove_file(DEV_UNIT_TEST_RESULT_DIRECTORY+file_name+".xml")
...
def execute_command(command) :
    ...
    #log += output
    if p_status == 0:
        logger.info("Command Execution Succesful"+"\n#######################################################\n")
    else:
...
def renameResourceAndServiceFolder() :
    Utility.change_directory(DEV_UNIT_TEST_RUN_DIRECTORY)
    os.rename("out/linux/x86_64/release/service",  "out/linux/x86_64/release/nonsecured_service")
    os.rename("out/linux/x86_64/release/resource", "out/linux/x86_64/release/nonsecured_resource")

def make_excel_report() :
    excel_report_command = "./devunittest_reporter.py --t %s --b %s --m %s"% (TARGET_SOURCE, BIT, BUILD_MODE)
...
if __name__ == '__main__':
    ...
    print("======================Start making excel report for Developer Unit TCs========================")
    #make_excel_report()
    print("======================Finished making excel report for Developer Unit TCs=====================")
    #write_log_into_file()
