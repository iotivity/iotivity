 ...
 */
//TODO: CBOR replace char* with OCRepPayload and remove payloadLen parameter
                                ...
                                char *payload, int payloadLen)
{
    ...
    //TODO: CBOR
    OCRepPayload secPayload = {};
    secPayload.base.type = PAYLOAD_TYPE_REPRESENTATION;
    secPayload.
    ...
    if ((gStateManager & SP_DISCOVERY_STARTED) && gToken)
    {
        ...
        if (0 == memcmp(gToken, responseInfo->info.token, responseInfo->info.tokenLength))
        {
            ...
            else
            {
                ...
                if(result == OC_STACK_OK && payload->type == PAYLOAD_TYPE_REPRESENTATION)
                {
                    ptrDoxm = NULL; //TODO:JSONToDoxmBin(((OCSecurityPayload*)payload)->securityData);
                }
    ...
    if ((gStateManager & SP_LIST_METHODS_STARTED) && gToken)
    {
        ...
        if (memcmp(gToken, responseInfo->info.token, responseInfo->info.tokenLength) == 0)
        {
            ...
            if (CA_SUCCESS == responseInfo->result)
            {
                ...
                if(result == OC_STACK_OK && payload->type == PAYLOAD_TYPE_REPRESENTATION)
                {
                    pstat =  NULL; //TODO JSONToPstatBin(((OCSecurityPayload*)payload)->securityData);
                }
        ...
        SPTargetDeviceInfo_t *deviceInfo, OicSecOxm_t selectedMethod)
{
    ...
    char *payload = NULL; //TODO: CBOR BinToDoxmJSON(deviceInfo->doxm);
    if (NULL == payload)
    {
                                    ...
                                    OicSecDpom_t selectedOperationMode)
{
    ...
    char *payloadBuffer = NULL; //TODO: BinToPstatJSON(deviceInfo->pstat);
                                  ...
                                  SPTargetDeviceInfo_t *selectedDeviceInfo)
{
    ...
    memcpy(selectedDeviceInfo->doxm->owner.id, provTooldeviceID.id , SVR_UUID_LENGTH);

    selectedDeviceInfo->doxm->owned = true;

    char *payloadBuffer =  NULL; //TODO:BinToDoxmJSON(selectedDeviceInfo->doxm);
    if (NULL == payloadBuffer)
    {
...
{
    char *credJson = NULL;
    credJson = NULL; //TODO:BinToCredJSON(cred);
    if (NULL == credJson)
    {
                        ...
                        OicSecAcl_t *acl)
{
    ...
    char *aclString = NULL;
    aclString = NULL; //TODO:BinToAclJSON(acl);
                                ...
                                SPTargetDeviceInfo_t *selectedDeviceInfo)
{
    ...
    char *payloadBuffer = NULL; //TODO:BinToPstatJSON(selectedDeviceInfo->pstat);
    if (NULL == payloadBuffer)
    {
        OC_LOG(ERROR, TAG, "Error while converting pstat bin to json");
        return SP_RESULT_INTERNAL_ERROR;
