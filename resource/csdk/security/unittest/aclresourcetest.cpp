...
TEST(ACLResourceTest, CBORDefaultACLConversion)
{
    ...
    uint8_t *defaultPsStorage = AclToCBORPayload(defaultAcl, &defaultAclSize);
...
TEST(ACLResourceTest, CBORACLConversion)
{
    ...
    size_t size = 0;
    uint8_t *psStorage = AclToCBORPayload(secAcl, &size);
...
TEST(ACLResourceTest, ACLDeleteWithSingleResourceTest)
{
    ...
    size_t size = 0;
    uint8_t  *payload = AclToCBORPayload(&acl, &size);
    ASSERT_TRUE(NULL != payload);
