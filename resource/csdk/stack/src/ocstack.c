                            ...
                            const CAResponseInfo_t *responseInfo)
{
    ...
    {
        result = ParsePayload(&response.payload,  responseInfo->info.payload,
...
void HandleCAResponses(const CAEndpoint_t* endPoint, const CAResponseInfo_t* responseInfo)
{
    ...
    if(cbNode)
    {
        ...
        else
        {
            ...
            if(responseInfo->info.payload &&
               responseInfo->info.payloadSize &&
               OC_STACK_OK != ParsePayload(&response.payload, responseInfo->info.payload,
                                           responseInfo->info.payloadSize))
            {
                            ...
                            uint8_t numOptions)
{
    ...
    {
        if((result =
            ConvertPayload(payload, &requestInfo.info.payload, &requestInfo.info.payloadSize))
                != OC_STACK_OK)
        {
