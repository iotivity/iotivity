        ...
        OCObservationId observeID)
{
    if (entityHandlerRequest)
    {
        ...
        {
            if(ParsePayload(&entityHandlerRequest->payload, payload, payloadSize) != OC_STACK_OK)
            {
...
OCStackResult HandleSingleResponse(OCEntityHandlerResponse * ehResponse)
{
    ...
    if(ehResponse->payload)
    {
        ...
        if((result = ConvertPayload(ehResponse->payload, &responseInfo.info.payload,
...
OCStackResult HandleAggregateResponse(OCEntityHandlerResponse * ehResponse)
{
    ...
    if(serverRequest)
    {
        ...
        printf("ERICH: Handle Aggregate Response!!!\n");
        exit(-1);
        // append
        // // TODO: ERICH: What is the correct logic here?!?
        // AppendPayload(&serverResponse->payload, ehResponse->payload);
