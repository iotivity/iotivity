...
static void CANFCPacketReceivedCB(const CAEndpoint_t *endpoint, const void *data,
                                  uint32_t dataLength);
...
void CANFCPacketReceivedCB(const CAEndpoint_t *endpoint, const void *data, uint32_t dataLength)
{
    OIC_LOG(DEBUG, TAG, "IN");
    ...
    {
        g_packetReceivedCallback(endpoint, data, dataLength);
    }
                           ...
                           CAErrorHandleCallback errorCallback, ca_thread_pool_t handle)
{
    ...
    CAConnectivityHandler_t nfcHandler;
    nfcHandler.startAdapter = CAStartNFC;
    nfcHandler.startListenServer = CAStartNFCListeningServer;
    nfcHandler.startDiscoveryServer = CAStartNFCDiscoveryServer;
    nfcHandler.sendData = CASendNFCUnicastData;
    nfcHandler.sendDataToAll = CASendNFCMulticastData;
    nfcHandler.GetnetInfo = CAGetNFCInterfaceInformation;
                           ...
                           uint32_t dataLength)
{
    ...
    CANFCData *nfcData = (CANFCData *) OICMalloc(sizeof(CANFCData));
    if (!nfcData)
    {
        OIC_LOG(ERROR, TAG, "Memory allocation failed!");
        return NULL;
...
{
    VERIFY_NON_NULL_VOID(nfcData, TAG, "nfcData is NULL");
