...
/**
 * @file CARAinterface.h
 * @brief This file provides APIs IP client/server/network monitor modules
 */

#ifndef CA_RA_INTERFACE_H_
#define CA_RA_INTERFACE_H_
 ...
 *
 * @return NONE
 * @pre  Callback must be registered using CARASetPacketReceiveCallback()
 */
typedef void (*CARAPacketReceivedCallback)(const char *ipAddress, uint16_t port,
 ...
 * @return NONE
 * @pre  Callback must be registered using CARASetExceptionCallback()
 */
typedef void (*CARAExceptionCallback)(CAAdapterServerType_t type);

/**
 * @brief  Initialize RA server
 *
 ...
 * @retval  #CA_STATUS_FAILED Operation failed
 */
CAResult_t CARAStartUnicastServer(const char *localAddress, uint16_t *port, bool forceStart,
                                  bool secured);
...
/**
 * @brief  Used to get the number of unicast RA servers currently running.
 *
 * @param   isSecured  [IN] To identify whether its secured unicast server or normal server.
 *
 * @return  positive value on success and -1 on error.
 */
int16_t CAGetNumberOfUnicastRAServers(bool isSecured);
 ...
 */
int CAGetSocketFdFromUnicastRAServer(const char *ipAddress, bool isSecured, bool isMulticast);
...
/**
 * @brief  Callback to be notified when RA adapter connection state changes.
 *
 * @param  status       [IN] Connection status either #CA_INTERFACE_UP or #CA_INTERFACE_DOWN.
 * @return  NONE
 * @pre  Callback must be registered using CARASetConnectionStateChangeCallback()
 */
typedef void (*CARAConnectionStateChangeCallback)(CANetworkStatus_t status);

/**
 * @brief Initialize RA network monitor
 *
 * @param  threadPool [IN] Thread pool for managing network monitor thread.
 * @param  callback handler for connection state changed.
 * *
 * @return  #CA_STATUS_OK or Appropriate error code
 * @retval  #CA_STATUS_OK  Successful
 * @retval  #CA_STATUS_INVALID_PARAM Invalid input data
 ...
 */
CAResult_t CARAGetInterfaceInfo(u_arraylist_t **netInterfaceList);
 ...
 */
CAResult_t CARAGetInterfaceSubnetMask(const char *ipAddress, char **subnetMask);
 ...
 * @return NONE
 */
void CARASetConnectionStateChangeCallback(CARAConnectionStateChangeCallback callback);

#ifdef __cplusplus
}
#endif

#endif //CA_RA_INTERFACE_H_
