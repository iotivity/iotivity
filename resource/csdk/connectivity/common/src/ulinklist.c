...
CAResult_t u_linklist_free(u_linklist_t **linklist)
{
    ...
    {
        OIC_LOG(ERROR, TAG, "Invalid list");
...
CAResult_t u_linklist_remove(u_linklist_t *linklist, u_linklist_iterator_t **iter)
{
    ...
    while(prev->next != NULL && prev->next != *iter)
    {
        prev = prev->next;
    }
...
uint32_t u_linklist_length(u_linklist_t *linklist)
{
    VERIFY_NON_NULL(linklist, TAG, "list is null");
...
void u_linklist_init_iterator(u_linklist_t *linklist, u_linklist_iterator_t **iter)
{
