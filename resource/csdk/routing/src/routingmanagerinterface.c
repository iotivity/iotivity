...
OCStackResult RMDiscoverGatewayResource()
{
    ...
    OCCallbackData gwData;
    gwData.cb = RMDiscoverGatewayCallback;
    gwData.context = NULL;
    gwData.cd = NULL;
    ...
    result = OCDoResource(NULL, OC_REST_DISCOVER, GW_RESOURCE_URI, 0, 0,
                 ...
                 OC_LOW_QOS, &gwData, NULL, 0);

    // Temp fix for packet drops in WIFI.
    for (int sendData = 0; sendData < 3; sendData++)
    {
        result = OCDoResource(NULL, OC_REST_DISCOVER, GW_RESOURCE_URI, 0, 0,
                     CT_ADAPTER_IP, OC_LOW_QOS, &gwData, NULL, 0);
        usleep(100000);
    }
...
OCStackResult RMSendObserveRequest(OCDevAddr devAddr, OCRepPayload *payload)
{
    OC_LOG(DEBUG, TAG, "RMSendObserveRequest IN");
    OC_LOG_V(DEBUG, TAG, "Destination address is %s:%d", devAddr.addr, devAddr.port);
    OCCallbackData observeData;
    observeData.cb = RMObserveRequestCallback;
    observeData.context = NULL;
    observeData.cd = NULL;
...
OCStackResult RMSendDeleteRequest(OCDevAddr devAddr, OCRepPayload *payload)
{
...
OCStackResult RMAddObserverToStack(const OCServerRequest *request, OCObservationId *obsID)
{
    ...
    result = AddObserver((const char*)(request->resourceUrl),
                ...
                *obsID, request->requestToken, request->tokenLength,
                (OCResource *)g_gateWayHandle, request->qos,
                &request->devAddr);
