...
// TODO::: this macro to be removed and compared with mCastSeqNumber for source gateway
/**
 * Maximum number of hopes after which multicast packet is dropped. Helps in finding loops.
 */
#define MAX_MULTICAST_HOP_COUNT 5
...
OCTransportFlags RMCAToOCTransportFlags(CATransportFlags_t caFlags)
{
    return (OCTransportFlags)caFlags;
}

void RMCopyDevAddrToEndpoint(const OCDevAddr *in, CAEndpoint_t *out)
{
    if (!in || !out)
    {
        OC_LOG(ERROR, TAG, "Invalid input arguements: in/out");
        return;
    }

    out->adapter = (CATransportAdapter_t)in->adapter;
    out->flags = RMOCToCATransportFlags(in->flags);
    strncpy(out->addr, in->addr, MAX_ADDR_STR_SIZE);
    memcpy(out->routeData, in->routeData, MAX_ADDR_STR_SIZE_CA);
    out->port = in->port;
}

void RMCopyEndpointToDevAddr(const CAEndpoint_t *in, OCDevAddr *out)
{
    if (!in || !out)
    {
        OC_LOG(ERROR, TAG, "Invalid input arguements: in/out");
    ...
    out->flags = RMCAToOCTransportFlags(in->flags);
    strncpy(out->addr, in->addr, MAX_ADDR_STR_SIZE);
...
void RMGenerateGatewayUUID()
{
    OC_LOG(DEBUG, TAG, "RMGenerateGatewayUUID IN");
    uint8_t gatewayArray[GATEWAY_ID_LENGTH] = { 0 };
    for (uint8_t i = 0; i < GATEWAY_ID_LENGTH; i++)
    {
        OCSeedRandom();
        while (0 == gatewayArray[i])
        {
            gatewayArray[i] = OCGetRandomByte();
        }
    }
    memcpy(&g_GatewayID, gatewayArray, GATEWAY_ID_LENGTH);

    OC_LOG_V(INFO, RM_TAG, "Gateway UUID: %u", g_GatewayID);
    OC_LOG(DEBUG, TAG, "RMGenerateGatewayUUID OUT");
}
...
OCStackResult RMInitialize()
{
    ...
    // Generates a 4 byte UUID for the Gateway resource.
    RMGenerateGatewayUUID();
    ...
    g_refreshTableTime = RTMGetCurrentTime();
...
OCStackResult RMHandleGatewayRequest(OCServerRequest *request, OCResource *resource)
{
...
OCStackResult RMHandleRequestPayload(OCDevAddr devAddr, uint8_t *reqPayload, size_t payloadSize)
{
    ...
    RMCopyDevAddrToEndpoint(&devAddr, &endpoint);
    ...
    RTMGatewayEntry_t *newNode =
        (RTMGatewayEntry_t *)OICMalloc(sizeof(RTMGatewayEntry_t));
    newNode->destination = (RTMGatewayId_t *)OICMalloc(sizeof(RTMGatewayId_t));
    newNode->routeCost = 1;
    newNode->destination->gatewayId = gatewayId;
...
OCStackResult RMHandleResponsePayload(OCDevAddr devAddr, OCRepPayload *respPayload)
{
    ...
    if (0 < seqNum)
    {
        ...
        if (OC_STACK_DUPLICATE_REQUEST == result)
        {
    ...
    while (NULL != iterTable)
    {
        ...
        // Check if the entry is its own.
        if (entry->destination->gatewayId == g_GatewayID)
        {
            OC_LOG(INFO, TAG, "Own entry, continue!!");
        ...
        OC_LOG_V(INFO, TAG, "Gateway ID: %u",
                 entry->destination->gatewayId);
...
OCStackResult RMAddObserver(OCServerRequest *request, OCObservationId *obsID)
{
    ...
    RMCopyDevAddrToEndpoint(&(request->devAddr), &endpoint);
    ...
    if (!g_isValidated && ROUTINGTABLE_REFRESH_TIMEOUT <= (currentTime - g_refreshTableTime))
    {
        ...
        if (0 < u_linklist_length(invalidInterfaces))
        {
            ...
            while (NULL != iterTable)
            {
                ...
                RMCopyEndpointToDevAddr(&(entry->destIntfAddr), &devAddr);
                RMSendObserveRequest(devAddr, NULL);
                u_linklist_get_next(&iterTable);
            }
        }
...
void RMSendDeleteToNeighbourNodes()
{
    ...
    while (NULL != iterTable)
    {
        ...
        for (uint32_t i = 0; i < u_arraylist_length(entry->destination->destIntfAddr); i++)
        {
            ...
            OCDevAddr devAddr = {.adapter = OC_DEFAULT_ADAPTER};
            RMCopyEndpointToDevAddr(&(dest->destIntfAddr), &devAddr);
...
OCStackResult RMHandlePacket(bool isRequest, void *message, CAEndpoint_t *sender,
                             bool *selfDestination)
{
    ...
    res = RMParseRouteOption(&info->options[routeIndex], &routeOption);
    if (OC_STACK_OK != res)
    {
    ...
    if (forward)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (CA_STATUS_OK != caRes)
                {
                    OC_LOG_V(ERROR, RM_TAG, "Failed to forward response to next hop [%d][%s]", caRes,
                                          nextHop.addr);
...
OCStackResult RMHandleRequest(CARequestInfo_t *message, CAEndpoint_t *sender,
                              bool *selfDestination)
{
...
OCStackResult RMHandleResponse(CAResponseInfo_t *message, CAEndpoint_t *sender,
                               bool *selfDestination)
{
