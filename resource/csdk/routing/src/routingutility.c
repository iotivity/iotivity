                        ...
                        uint8_t *numOptions)
{
    ...
    else
    {
        ...
        if (!optionPtr)
        {
            OC_LOG(ERROR, TAG, "Malloc failed");
    ...
    if (*numOptions != index)
    {
        ...
        if (OC_STACK_OK != res)
        {
            OC_LOG_V(ERROR, TAG, "RMParseRouteOption failed");
    ...
    if(destination)
    {
        memcpy(&(routeOption.destGw), destination, GATEWAY_ID_LENGTH);
        memcpy(&(routeOption.destEp), destination + GATEWAY_ID_LENGTH, ENDPOINT_ID_LENGTH);
    }
    ...
    if (gatewayId)
    {
        memcpy(&(routeOption.srcGw), &gatewayId, GATEWAY_ID_LENGTH);
    }
    ...
    if (OC_STACK_OK != res)
    {
        OC_LOG_V(ERROR, TAG, "Creation of routing option failed");
        OICFree(optionPtr);
        return res;
    }
    ...
    if (0 != (*options + routeIndex)->optionLength)
    {
        ...
        if (0 < sLen)
        {
            memcpy(endpoint->routeData, (*options + routeIndex)->optionData + count,
                   GATEWAY_ID_LENGTH);
            OC_LOG_V(DEBUG, TAG, "adding srcgid: %u in endpoint [%d]", *((uint32_t *)endpoint->routeData), sLen);
            ...
            if (GATEWAY_ID_LENGTH < sLen)
            {
                ...
                OC_LOG_V(DEBUG, TAG, "adding srceid: %u in endpoint", *((uint16_t *)(endpoint->routeData + GATEWAY_ID_LENGTH)));
            }
        }
    }
...
OCStackResult RMCreateRouteOption(const RMRouteOption_t *optValue, CAHeaderOption_t *options)
{
    ...
    char *tempData = OICCalloc(totalLength, sizeof(char));
    if (NULL == tempData)
    {
