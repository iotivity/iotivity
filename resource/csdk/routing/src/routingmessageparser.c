 ...
 */
#define UPDATE_SEQ_NUM      "updateseqnum"
                                            ...
                                            OCRepPayload **payload)
{
    ...
    *payload =  (OCRepPayload *)OCRepPayloadCreate();
    ...
    int len = u_linklist_length(routingtable);
    ...
    if (i > 0)
    {
        ...
        if (!res)
        {
            OC_LOG(ERROR, TAG, PCF("Failed to Construct Discover Payload"));
            return OC_STACK_ERROR;
        }
    }
                                         ...
                                         OCRepPayload **removedPayload)
{
    ...
    *removedPayload =  OCRepPayloadCreate();
    ...
    int len = u_linklist_length(removedGateways);
    const OCRepPayload *arrayPayload[len];
    ...
    if (!res)
    {
        OC_LOG(ERROR, TAG, PCF("Failed to Construct Removal Payload"));
        return OC_STACK_ERROR;
    }
